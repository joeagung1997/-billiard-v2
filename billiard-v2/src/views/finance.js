...
export function financeMenuPage(role = "owner", items = [], toppings = [], hasErr = false, editItem = null, extras = {}) {
  ...
  const js = ""
    ...
    // ── Resep editor JS (defensive: event delegation + window.* globals) ──
    // Diwrap dlm IIFE + try/catch supaya error di sini gak break script lain.
    // rsAddRow/rsDelRow expose ke window biar inline onclick="..." berfungsi.
    + "function rpFmt(n){n=Math.round(n||0);var sign=n<0?'-':'';n=Math.abs(n);return sign+'Rp '+String(n).replace(/\\B(?=(\\d{3})+(?!\\d))/g,'.');}"
    + "(function(){try{"
    +   "if(!document.getElementById('resepEditor'))return;"
    +   "function recompute(){"
    ...
    +       "if(!sel||!qin)return;"
    +       "var opt=sel.options[sel.selectedIndex];"
    +       "var hp=(opt&&opt.dataset)?parseInt(opt.dataset.hpporsi||0):0;"
    +       "var label=(opt&&opt.dataset)?(opt.dataset.label||''):'';"
    +       "var q=parseFloat((qin.value||'').toString().replace(',','.'))||0;"
    +       "var s=Math.round(hp*q);"
    +       "if(sub){sub.textContent=rpFmt(s);sub.dataset.sub=s;}"
    ...
    +   "}"
    +   "function onRowEv(t){"
    +     // Pilih bahan & qty masih kosong/0 → auto-set qty=1 (UX biar HPP langsung tampil).
    +     "if(t&&t.matches&&t.matches('select.mp-rs-bahan')&&t.value){"
    +       "var tr=t.closest('tr');"
    +       "if(tr){var qin=tr.querySelector('input.mp-rs-qty');if(qin&&(!qin.value||parseFloat(qin.value)===0))qin.value='1';}"
    +     "}"
    +     "recompute();"
    +   "}"
    +   // Event delegation di document — tangkap change/input dari semua row,
    +   // termasuk row dinamis dari rsAddRow. Tidak perlu rebind manual.
    +   "document.addEventListener('change',function(ev){"
    +     "if(ev.target&&ev.target.closest&&ev.target.closest('#resepRows'))onRowEv(ev.target);"
    +   "});"
    +   "document.addEventListener('input',function(ev){"
    +     "if(ev.target&&ev.target.closest&&ev.target.closest('#resepRows'))onRowEv(ev.target);"
    +   "});"
    +   // Expose rsAddRow & rsDelRow ke window biar inline onclick=... bisa panggil.
    +   "window.rsAddRow=function(){"
    +     "try{"
    ...
    +     "}catch(e){console.error('rsAddRow:',e);}"
    +   "};"
    +   "window.rsDelRow=function(btn){"
    +     "try{"
    +       "var tr=btn.closest('tr');var tbody=document.getElementById('resepRows');"
    +       "if(tbody.querySelectorAll('.mp-rs-trow').length>1){tr.remove();}"
    +       "else{var s=tr.querySelector('select');var q=tr.querySelector('input');if(s)s.value='';if(q)q.value='';}"
    +       "recompute();"
    +     "}catch(e){console.error('rsDelRow:',e);}"
    +   "};"
    +   "recompute();"
    + "}catch(e){console.error('resepEditor init:',e);}})();"
