...
export function financeDashboard({ transaksi, token, role = "owner", displayName = "", bulanFilter, jenisFilter, tglDari, tglSampai, kategoriList = [], subKategoriList = [], menuItems = [], toppings = [], msg = "" }) {
  ...
  return docHeadV4("Keuangan")
    ...
    + "<select class=\"fsel\" id=\"wizDurasi\" onchange=\"wizDurasiChange(this.value)\"><option value=\"\">Pilih durasi...</option><option>Open / Loss</option><option>1 Jam</option><option>2 Jam</option><option>3 Jam</option><option>4 Jam</option><option>5 Jam</option><option>6 Jam</option><option>7 Jam</option><option>8 Jam</option></select>"
    ...
    + "if(!dr||!jEl)return;"
    + "var v=dr.value;"
    + "if(v==='Open / Loss'){jEl.value='';wizHideErr('wizJumlahErr');return;}"
    + "var m=v&&v.match(/^(\\d+) Jam$/);"
    + "if(!m)return;"
    + "var jam=parseInt(m[1]);var rate=wizS.waktu==='malam'?12000:10000;"
    + "jEl.value=String(jam*rate);wizFmtJ(jEl);wizHideErr('wizJumlahErr');}"
