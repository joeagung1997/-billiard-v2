...
export function financeDashboard({ transaksi, token, bulanFilter, jenisFilter, tglDari, tglSampai, kategoriList = [], subKategoriList = [], menuItems = [], toppings = [] }) {
  ...
  return docHeadV4("Keuangan")
    ...
    + "<input class=\"fin-pfx-inp\" type=\"text\" inputmode=\"numeric\" id=\"wizJumlah\" placeholder=\"0\" oninput=\"wizFmtJ(this);wizCalcKembalian()\"></div>"
    ...
    + "</div>"
    + "<div id=\"wizKembalianWrap\" style=\"display:none\">"
    + "<div class=\"frow\" style=\"gap:10px\">"
    + "<div class=\"fmg\" style=\"flex:1\"><label class=\"fin-wiz-lbl\">Uang Diterima (Rp)</label>"
    + "<div class=\"fin-inp-pfx\"><span class=\"fin-pfx-lbl\">Rp</span>"
    + "<input class=\"fin-pfx-inp\" type=\"text\" inputmode=\"numeric\" id=\"wizBayar\" placeholder=\"0\" oninput=\"wizFmtBayar(this)\"></div></div>"
    + "<div class=\"fmg\" style=\"flex:1\"><label class=\"fin-wiz-lbl\">Saldo Awal Kas <span style=\"font-weight:400;font-size:10px;text-transform:none;letter-spacing:0;color:#b0bfae\">(opsional)</span></label>"
    + "<div class=\"fin-inp-pfx\"><span class=\"fin-pfx-lbl\">Rp</span>"
    + "<input class=\"fin-pfx-inp\" type=\"text\" inputmode=\"numeric\" id=\"wizSaldoAwal\" placeholder=\"0\" oninput=\"wizFmtSaldoAwal(this)\"></div></div>"
    + "</div>"
    + "<div id=\"wizKembalianResult\" style=\"display:none;flex-direction:column;gap:6px;padding:10px 12px;border-radius:8px;background:var(--surface2);margin:-4px 0 8px\">"
    + "<div style=\"display:flex;align-items:center;gap:8px\"><i class=\"ti ti-arrow-back-up\" style=\"font-size:14px;color:var(--txt3)\"></i><span style=\"font-size:12px;color:var(--txt2)\">Kembalian</span><span id=\"wizKembalianAmt\" style=\"font-size:14px;font-weight:700;font-family:var(--ff-mono);margin-left:auto\"></span></div>"
    + "<div id=\"wizSaldoAkhirRow\" style=\"display:none;align-items:center;gap:8px\"><i class=\"ti ti-wallet\" style=\"font-size:14px;color:var(--txt3)\"></i><span style=\"font-size:12px;color:var(--txt2)\">Saldo Akhir Kas</span><span id=\"wizSaldoAkhirAmt\" style=\"font-size:14px;font-weight:700;font-family:var(--ff-mono);margin-left:auto\"></span></div>"
    + "</div>"
    + "</div>"
    ...
    + "<div class=\"fin-sb-row\" id=\"sumKembalianRow\" style=\"display:none\"><span>Kembalian</span><span class=\"fin-sb-val\" id=\"sumKembalian\" style=\"font-weight:700\">Rp 0</span></div>"
    + "</div></div>"
    ...
    + "var kw=document.getElementById('wizKembalianWrap');if(kw)kw.style.display=wizS.tipe==='income'?'':'none';"
    + "var bEl=document.getElementById('wizBayar');if(bEl)bEl.value='';"
    + "var sEl=document.getElementById('wizSaldoAwal');if(sEl)sEl.value='';"
    + "var kr=document.getElementById('wizKembalianResult');if(kr)kr.style.display='none';}"
    ...
    + "wizUpdateDateDisplay();}"
    + "var kw=document.getElementById('wizKembalianWrap');if(kw)kw.style.display=t==='income'?'':'none';"
    + "if(t==='expense'){var bEl=document.getElementById('wizBayar');if(bEl)bEl.value='';"
    + "var sEl2=document.getElementById('wizSaldoAwal');if(sEl2)sEl2.value='';"
    + "var kr=document.getElementById('wizKembalianResult');if(kr)kr.style.display='none';}}"
    ...
    + "jEl.value=s;}"
    + "wizCalcKembalian();wizHideErr('wizJumlahErr');}"
    ...
    + "function wizFmtBayar(el){var raw=el.value.replace(/\\D/g,'');var n=parseInt(raw)||0;var s=n>0?String(n):'';el.value=s?s.replace(/\\B(?=(\\d{3})+(?!\\d))/g,'.'):'';wizCalcKembalian();}"
    + "function wizFmtSaldoAwal(el){var raw=el.value.replace(/\\D/g,'');var n=parseInt(raw)||0;var s=n>0?String(n):'';el.value=s?s.replace(/\\B(?=(\\d{3})+(?!\\d))/g,'.'):'';wizCalcKembalian();}"
    + "function wizCalcKembalian(){"
    + "var wrap=document.getElementById('wizKembalianWrap');"
    + "if(!wrap||wrap.style.display==='none')return;"
    + "var jRaw=(document.getElementById('wizJumlah')?document.getElementById('wizJumlah').value:'').replace(/\\./g,'');"
    + "var bRaw=(document.getElementById('wizBayar')?document.getElementById('wizBayar').value:'').replace(/\\./g,'');"
    + "var sRaw=(document.getElementById('wizSaldoAwal')?document.getElementById('wizSaldoAwal').value:'').replace(/\\./g,'');"
    + "var total=parseInt(jRaw)||0;var bayar=parseInt(bRaw)||0;var saldo=parseInt(sRaw)||0;"
    + "var el=document.getElementById('wizKembalianResult');var amt=document.getElementById('wizKembalianAmt');"
    + "var salRow=document.getElementById('wizSaldoAkhirRow');var salAmt=document.getElementById('wizSaldoAkhirAmt');"
    + "if(!el||!amt)return;"
    + "if(bayar<=0){el.style.display='none';if(salRow)salRow.style.display='none';return;}"
    + "el.style.display='flex';var kemb=bayar-total;"
    + "if(kemb<0){amt.textContent='Kurang Rp '+Math.abs(kemb).toLocaleString('id-ID');amt.style.color='#ef4444';}"
    + "else{amt.textContent='Rp '+kemb.toLocaleString('id-ID');amt.style.color='#22c55e';}"
    + "if(salRow&&salAmt){"
    + "if(saldo>0&&kemb>=0){salRow.style.display='flex';"
    + "var cukup=saldo>=kemb;var akhir=saldo+total;"
    + "if(cukup){salAmt.textContent='Rp '+akhir.toLocaleString('id-ID');salAmt.style.color='var(--accent)';}"
    + "else{salAmt.textContent='\\u26A0 Kas kurang Rp '+(kemb-saldo).toLocaleString('id-ID')+' utk kembalian';salAmt.style.color='#ef4444';}}"
    + "else{salRow.style.display='none';}}}"
    ...
    + "disp.textContent=HARI[d.getDay()]+', '+D+' '+BULAN[M]+' '+Y+(jam?' \\u00B7 '+jam:'');}"
    + "function wizGoTo(n){"
    + "wizS.step=n;"
    ...
    + "var bRaw2=(document.getElementById('wizBayar')?document.getElementById('wizBayar').value:'').replace(/\\./g,'');"
    + "var bayar2=parseInt(bRaw2)||0;var kemb2=bayar2-(parseInt(jRaw)||0);"
    + "var skr=document.getElementById('sumKembalianRow');var ska=document.getElementById('sumKembalian');"
    + "if(skr)skr.style.display=wizS.tipe==='income'&&bayar2>0?'':'none';"
    + "if(ska&&bayar2>0){ska.textContent='Rp '+kemb2.toLocaleString('id-ID');ska.style.color=kemb2>=0?'var(--green)':'#ef4444';}"
    + "}"
