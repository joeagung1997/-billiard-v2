...
export function financeSesiPage({ role = "owner", displayName = "", sesiList = [], mejaTersedia = [], menuItems = [], msg = "" }) {
  ...
  const js =
    ...
    + "function bukaCalc(){var ms=document.getElementById('bukaMeja');if(!ms)return;var o=ms.options[ms.selectedIndex];var dur=document.getElementById('bukaDurasi').value;var el=document.getElementById('bukaPrice');if(!o||!o.value){el.textContent='—';return;}if(dur==='Open'){el.textContent='Diisi saat tutup';return;}var m=dur.match(/^(\\d+) Jam$/);if(!m){el.textContent='—';return;}var jam=parseInt(m[1]);var si=parseInt(o.dataset.ts||'0')||0,ma=parseInt(o.dataset.tm||'0')||0;var st=bukaStartMs();el.textContent='Rp '+tarifSplit(st,st+jam*3600000,si,ma).toLocaleString('id-ID');}"
