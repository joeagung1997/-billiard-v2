...
export function financeDashboard({ transaksi, token, bulanFilter, jenisFilter, tglDari, tglSampai, kategoriList = [], subKategoriList = [], menuItems = [], toppings = [] }) {
  ...
  return docHeadV4("Keuangan")
    ...
    + "function saveSaldoKas(){var v=(document.getElementById('finSaldoKas')||{}).value||'';try{localStorage.setItem('fin_saldo_kas',v);}catch(e){}}"
    + "(function(){try{var v=localStorage.getItem('fin_saldo_kas');var el=document.getElementById('finSaldoKas');if(el&&v)el.value=v;}catch(e){}})();"
    + "</script>"
