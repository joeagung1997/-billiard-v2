...
export function financeDashboard({ transaksi, token, role = "owner", displayName = "", bulanFilter, jenisFilter, tglDari, tglSampai, kategoriList = [], subKategoriList = [], menuItems = [], toppings = [], msg = "" }) {
  ...
  return docHeadV4("Keuangan")
    ...
    + "<select class=\"fsel\" id=\"wizDurasi\" onchange=\"wizDurasiChange(this.value)\"><option value=\"\">Pilih durasi...</option><option>1 Jam</option><option>2 Jam</option><option>3 Jam</option><option>4 Jam</option><option>5 Jam</option><option>6 Jam</option><option>7 Jam</option><option>8 Jam</option><option>Open</option><option>Loss</option></select>"
    ...
    + "<div id=\"wizDetailMainErr\" style=\"display:none;font-size:11px;color:#a32d2d;margin-top:4px\">Detail main wajib diisi untuk durasi Open atau Loss.</div>"
    + "</div>"
    + "</div>"
    ...
    + "if(dr&&(dr.value==='Open'||dr.value==='Loss')&&dtl&&dtl.value.trim())ket+=' ('+dtl.value.trim()+')';}"
    ...
    + "if(el)el.style.display=(v==='Open'||v==='Loss')?'':'none';"
    ...
    + "if(dr.value==='Open'||dr.value==='Loss'){"
    + "var dtl=document.getElementById('wizDetailMainInp');"
    + "if(!dtl||!dtl.value.trim()){wizShowErr('wizDetailMainErr','Detail main wajib diisi untuk durasi Open atau Loss.');if(dtl)dtl.focus();return;}}}"
