...
// Bagi PEMASUKAN jadi Cash vs QRIS (GROSS, per metode bayar) — utk kartu
// "CASH (LACI)" & "QRIS" di Dashboard. Definisi owner: "Pemasukan = total cash
// + QRIS", "Cash/laci = berapa yg dibayar customer pakai tunai". Maka:
//   • hanya baris jenis 'pemasukan' (penjualan) — TIDAK dikurangi pengeluaran,
//   • 'qris' → QRIS; selain itu (termasuk bayar kosong) → Cash, supaya tidak
//     ada transaksi yg hilang → cash + qris === total pemasukan.
// Pemanggil tanggung jawab nyaring tukar/void/belum-lunas (mis. revenueSortedTbl).
// Pure — bisa diuji tanpa DB.
export function bagiPemasukanMetode(rows = []) {
  let cash = 0, qris = 0;
  for (const t of (rows || [])) {
    if (!t || t.jenis !== "pemasukan") continue;
    if (t.bayar === "qris") qris += (t.jumlah || 0);
    else                    cash += (t.jumlah || 0);
  }
  return { cash, qris };
}
...
export function financeDashboard({ transaksi, token, role = "owner", displayName = "", shift = "siang", bulanFilter, jenisFilter, tglDari, tglSampai, kategoriList = [], subKategoriList = [], menuItems = [], toppings = [], mejaList = [], accountsAll = [], karyawanAll = [], analisis = null, msg = "", transaksiOnly = false }) {
  ...
  // Opsi meja utk dropdown Catat Transaksi — dari master Manajemen Meja (hanya
  // status 'aktif'; maintenance/nonaktif sudah difilter di readMejaAktif). Tiap
  // option bawa tarif (data-ts/tm/to) utk auto-calc harga per meja di wizard.
  const mejaOptsHtml = mejaList.map((m) =>
    "<option value=\"" + escHtml(m.nama) + "\""
    + " data-ts=\"" + (m.tarif_siang || 0) + "\""
    + " data-tm=\"" + (m.tarif_malam || 0) + "\""
    + " data-to=\"" + (m.tarif_open  || 0) + "\">"
    + escHtml(m.nama) + "</option>"
  ).join("");
  ...
  // CASH (LACI) & QRIS = PEMASUKAN (penjualan) per metode bayar — GROSS, bukan
  // saldo bersih. Owner: "Pemasukan = total cash + QRIS", "Cash/laci = berapa
  // yg dibayar customer pakai tunai". Basis = revenueSortedTbl (non-tukar,
  // lunas, ikut scope tgl) — sama dgn kartu Pemasukan (chartTotalIn), jadi
  // totalCash + totalQris === chartTotalIn dan konsisten dgn POSISI KAS di
  // halaman Transaksi. TIDAK dikurangi pengeluaran (itu ada di Saldo Bersih).
  const { cash: totalCash, qris: totalQris } = bagiPemasukanMetode(revenueSortedTbl);
  ...
  const dashExtraCss = [
    ...
    // Tombol pilihan 3-kolom (Jenis Aktivitas Pemasukan: Sewa Meja / Kopi / Lainnya)
    ".mm-tog-3 .mm-tog-btn{font-size:11px;padding:8px 4px;gap:3px}",
    ".mm-tog-3 .mm-tog-btn i{font-size:13px}",
    // Subtotal F&B di dalam modal manual
    ...
    // z-index 10001: di atas .mm-overlay (Transaksi Manual, 10000) supaya pemilih
    // meja/durasi/F&B muncul di atas modal. Tetap di atas .overlay wizard (999).
    ".mip-overlay{display:none;position:fixed;top:0;left:0;right:0;height:100vh;height:100dvh;z-index:10001;background:rgba(15,23,42,.55);backdrop-filter:blur(3px);align-items:flex-end;justify-content:center;animation:mipFadeIn .2s ease}",
  ...
  return docHeadV4("Keuangan")
    ...
    + "<div class=\"fin-bayar-sub\">Dibayar tunai oleh customer</div>"
    + "</div>"
    + "<div class=\"fin-bayar-item\">"
    ...
    + "<div class=\"fin-bayar-sub\">Dibayar via QRIS oleh customer</div>"
    + "</div>"
    + "</div>"
    ...
    + (transaksiOnly
      ? "<div class=\"trx-summary-grid\">"
        ...
        +     "<div class=\"trx-sum-cell-lbl\">Total Transaksi <span class=\"trx-sum-ctx\" id=\"ctxTotal\"></span></div>"
        +   "</div>"
        ...
        +     "<div class=\"trx-sum-cell-lbl\">Pemasukan <span class=\"trx-sum-ctx\" id=\"ctxIn\"></span></div>"
        +   "</div>"
        ...
        +     "<div class=\"trx-sum-cell-lbl\">Pengeluaran <span class=\"trx-sum-ctx\" id=\"ctxOut\"></span></div>"
        +   "</div>"
        ...
        +     "<div class=\"trx-sum-cell-lbl\">Net / Saldo Periode <span class=\"trx-sum-ctx\" id=\"ctxNet\"></span></div>"
        +   "</div>"
    ...
    + (isOwner
      ? "<div class=\"mm-overlay\" id=\"mManual\" onclick=\"if(event.target===this)closeManual()\"><div class=\"mm-card\">"
        ...
        // Jenis Aktivitas (khusus Pemasukan) — Sewa Meja / Kopi-Snack / Lainnya
        +   "<div id=\"mmActWrap\">"
        +     "<label class=\"mm-lbl\">Jenis Aktivitas</label>"
        +     "<div class=\"mm-tog mm-tog-3\" id=\"mmActTog\">"
        +       "<button type=\"button\" class=\"mm-tog-btn active\" data-a=\"sewa\" onclick=\"mmSetAct('sewa')\"><i class=\"ti ti-device-gamepad-2\"></i> Sewa Meja</button>"
        +       "<button type=\"button\" class=\"mm-tog-btn\" data-a=\"kopi\" onclick=\"mmSetAct('kopi')\"><i class=\"ti ti-coffee\"></i> Kopi/Snack</button>"
        +       "<button type=\"button\" class=\"mm-tog-btn\" data-a=\"lainnya\" onclick=\"mmSetAct('lainnya')\"><i class=\"ti ti-dots-circle-horizontal\"></i> Lainnya</button>"
        +     "</div>"
        +   "</div>"
        ...
        +   "<div id=\"mmSewaWrap\">"
        +     "<div class=\"mm-row\">"
        +       "<div><label class=\"mm-lbl\">Nomor Meja</label>"
        +         "<div class=\"mip-wrap\">"
        +           "<select class=\"fsel mip-sel\" id=\"mmMeja\" onchange=\"mmHideErr('mmMejaErr');mmComputeBilliardPrice()\"><option value=\"\">Pilih meja...</option>" + mejaOptsHtml + "</select>"
        +           "<button type=\"button\" class=\"mip-btn\" onclick=\"openItemPicker(this,'Pilih Meja',true)\"><span class=\"mip-text\">Pilih meja...</span><i class=\"ti ti-chevron-down\" style=\"font-size:14px;color:var(--txt3);flex-shrink:0\"></i></button>"
        +         "</div>"
        +         "<div id=\"mmMejaErr\" class=\"mm-err\" style=\"display:none\">Meja wajib dipilih.</div>"
        +       "</div>"
        +       "<div><label class=\"mm-lbl\">Durasi (jam main)</label>"
        +         "<div class=\"mip-wrap\">"
        +           "<select class=\"fsel mip-sel\" id=\"mmDurasi\" onchange=\"mmDurasiChange(this.value)\"><option value=\"\">Pilih durasi...</option><option>Open / Loss</option><option>1 Jam</option><option>2 Jam</option><option>3 Jam</option><option>4 Jam</option><option>5 Jam</option><option>6 Jam</option><option>7 Jam</option><option>8 Jam</option></select>"
        +           "<button type=\"button\" class=\"mip-btn\" onclick=\"openItemPicker(this,'Pilih Durasi',true)\"><span class=\"mip-text\">Pilih durasi...</span><i class=\"ti ti-chevron-down\" style=\"font-size:14px;color:var(--txt3);flex-shrink:0\"></i></button>"
        +         "</div>"
        +         "<div id=\"mmDurasiErr\" class=\"mm-err\" style=\"display:none\">Durasi wajib dipilih.</div>"
        +       "</div>"
        +     "</div>"
        +     "<div id=\"mmOpenHint\" style=\"display:none;font-size:11px;color:#2660a4;background:rgba(38,96,164,.07);border:1px solid rgba(38,96,164,.18);border-radius:7px;padding:7px 10px;margin-bottom:12px;line-height:1.4\"></div>"
        +     "<div id=\"mmDetailMain\" style=\"display:none\"><label class=\"mm-lbl\">Detail Main <span style=\"color:#a32d2d\">*</span></label><input type=\"text\" id=\"mmDetailMainInp\" class=\"mm-inp\" style=\"margin-bottom:4px\" placeholder=\"contoh: 1 jam 30 menit, 3 orang...\"><div id=\"mmDetailMainErr\" class=\"mm-err\" style=\"display:none\">Detail main wajib diisi untuk durasi Open / Loss.</div></div>"
        +     "<label class=\"wiz-addon-toggle\" id=\"mmKopiAddonLbl\" style=\"margin-bottom:12px\"><input type=\"checkbox\" id=\"mmKopiAddonCb\" onchange=\"mmToggleAddon(this)\"><span><i class=\"ti ti-coffee\"></i>Sekalian pesan minum/makan?</span></label>"
        +   "</div>"
        // ── Pemasukan / Kopi-Snack (item F&B). Juga dipakai sbg tambahan Sewa Meja ──
        +   "<div id=\"mmKopiWrap\" style=\"display:none\">"
        +     "<label class=\"mm-lbl\" id=\"mmKopiLbl\">Item Pesanan</label>"
        +     "<div class=\"fin-menu-items\" id=\"mmMenuItems\">"
        +       "<div class=\"fin-menu-row\">"
        +         "<div class=\"mip-wrap\">"
        +           "<select class=\"fsel mip-sel\" onchange=\"mmItemChange(this)\"><option value=\"\">Pilih item...</option>" + menuOptsHtml + "</select>"
        +           "<button type=\"button\" class=\"mip-btn\" onclick=\"openItemPicker(this)\"><span class=\"mip-text\">Pilih item...</span><i class=\"ti ti-chevron-down\" style=\"font-size:14px;color:var(--txt3);flex-shrink:0\"></i></button>"
        +         "</div>"
        +         "<div class=\"fin-qty-stepper\">"
        +           "<button type=\"button\" class=\"fin-qty-btn\" onclick=\"mmQtyMinus(this)\" aria-label=\"Kurangi\">−</button>"
        +           "<input type=\"number\" class=\"fin-qty-inp\" value=\"1\" min=\"1\" inputmode=\"numeric\" oninput=\"mmCalcAddon()\">"
        +           "<button type=\"button\" class=\"fin-qty-btn\" onclick=\"mmQtyPlus(this)\" aria-label=\"Tambah\">+</button>"
        +         "</div>"
        +         "<button type=\"button\" class=\"fin-btn-rm-row\" onclick=\"mmRmItem(this)\"><i class=\"ti ti-x\"></i></button>"
        ...
        +             "<button type=\"button\" data-temp=\"ice\" onclick=\"mmSetTemp(this,'ice')\" style=\"padding:3px 10px;border-radius:6px;font-size:11px;cursor:pointer;border:1px solid #3b82f6;background:rgba(59,130,246,.12);color:#3b82f6;font-family:inherit\">❄ Ice</button>"
        +             "<button type=\"button\" data-temp=\"hot\" onclick=\"mmSetTemp(this,'hot')\" style=\"padding:3px 10px;border-radius:6px;font-size:11px;cursor:pointer;border:1px solid var(--border2);background:transparent;color:var(--txt2);font-family:inherit\">☕ Hot</button>"
        +           "</div>"
        +           "<div class=\"wiz-tops\" style=\"display:none;flex-direction:column;gap:4px\"></div>"
        +         "</div>"
        +       "</div></div>"
        +     "<button type=\"button\" class=\"fin-btn-add-item\" onclick=\"mmAddItem()\" style=\"margin-bottom:8px\"><i class=\"ti ti-plus\" style=\"font-size:14px\"></i> Tambah Item</button>"
        +     "<div id=\"mmKopiErr\" class=\"mm-err\" style=\"display:none\">Pilih minimal 1 item pesanan.</div>"
        +     "<div class=\"mm-sub\" id=\"mmKopiSub\" style=\"display:none\"></div>"
        +   "</div>"
        // ── Pemasukan / Lainnya (kategori bebas: Turnamen, dll) + Pengeluaran ──
        +   "<div id=\"mmLainnyaWrap\" style=\"display:none\">"
        +     "<label class=\"mm-lbl\">Kategori</label>"
        +     "<select id=\"mmKategori\" class=\"mm-inp\">" + modalGrpIn + "</select>"
        +   "</div>"
        ...
        +   "<textarea id=\"mmKet\" class=\"mm-inp mm-ta\" maxlength=\"200\" placeholder=\"mis. Karyawan lupa input sesi malam Meja 1\"></textarea>"
        // Field tersembunyi — diisi manualSubmit() sesuai mode aktif
        +   "<input type=\"hidden\" name=\"kategori\" id=\"mmKatHidden\">"
        +   "<input type=\"hidden\" name=\"keterangan\" id=\"mmKetFinal\">"
        +   "<input type=\"hidden\" name=\"kopi_keterangan\" id=\"mmKopiKet\">"
        +   "<input type=\"hidden\" name=\"kopi_jumlah\" id=\"mmKopiJ\">"
    ...
    + "<div class=\"fin-tog-3\">"
    + "<button type=\"button\" class=\"fin-tog-btn\" onclick=\"window.location.href='/operasional/sesi'\" title=\"Sewa meja dikelola lewat Sesi Meja (bisa tambah F&amp;B &amp; bayar per item)\"><i class=\"ti ti-receipt\"></i>Sewa Meja → Sesi</button>"
    + "<button type=\"button\" class=\"fin-tog-btn sel\" id=\"wiz-kopi\" onclick=\"wizSetAct('kopi')\"><i class=\"ti ti-coffee\"></i>Kopi / Snack</button>"
    ...
    + "<div class=\"mip-wrap\">"
    +   "<select class=\"fsel mip-sel\" id=\"wizMeja\" onchange=\"wizHideErr('wizMejaErr');wizComputeBilliardPrice()\"><option value=\"\">Pilih meja...</option>" + mejaOptsHtml + "</select>"
    ...
    + "</div>"
    + "</div>"
    + "<div id=\"wizOpenHint\" style=\"display:none;font-size:11px;color:#2660a4;background:rgba(38,96,164,.07);border:1px solid rgba(38,96,164,.18);border-radius:7px;padding:7px 10px;margin-bottom:8px;line-height:1.4\"></div>"
    ...
    +   "var _ctxParts=[];"
    +   "var _dateMap={today:'Hari Ini',yesterday:'Kemarin','this-week':'Minggu Ini','this-month':'Bulan Ini',custom:(dari&&sampai?dari+' s/d '+sampai:'Custom'),all:''};"
    +   "var _dateLbl=_dateMap[f]||'';"
    +   "if(_dateLbl)_ctxParts.push(_dateLbl);"
    +   "if(fKat)_ctxParts.push(fKat);"
    +   "var _sumMap={billiard:'Billiard',warkop:'Warkop',lain:'Lain-lain'};"
    +   "if(fSum&&_sumMap[fSum])_ctxParts.push(_sumMap[fSum]);"
    +   "if(fKru)_ctxParts.push(fKru);"
    +   "var _bayMap={cash:'Cash',qris:'QRIS'};"
    ...
    + "var mmAct='sewa';var mmAddonTotal=0;var mmAddonKet='';"
    // Baris tambahan F&B (suhu/topping) versi manual — pakai fungsi mm* (bukan wiz*)
    + "var MM_EXTRAS_HTML="
    + safeJson(
        '<div class="wiz-extras" style="grid-column:1/-1;display:none;flex-direction:column;gap:6px;padding:6px 0 2px">'
        + '<div class="wiz-temp" style="display:none;flex-direction:row;gap:6px;align-items:center">'
    ...
    +   "mmResetMeja();mmResetItems();"
    +   "var j=document.getElementById('mmJumlah');if(j)j.value='';"
    +   "var k=document.getElementById('mmKet');if(k)k.value='';"
    +   "manualSetBayar('cash');manualSetJenis('pemasukan');"
    +   "ov.classList.add('open');}"
    + "function mmResetMeja(){['mmMeja','mmDurasi'].forEach(function(id){var s=document.getElementById(id);if(s){s.selectedIndex=0;s.value='';var disp=s.parentElement.querySelector('.mip-text');if(disp){disp.textContent=(id==='mmMeja'?'Pilih meja...':'Pilih durasi...');disp.classList.remove('mip-text-filled');}}});"
    +   "var dm=document.getElementById('mmDetailMain');if(dm)dm.style.display='none';"
    +   "var dmi=document.getElementById('mmDetailMainInp');if(dmi)dmi.value='';"
    +   "var oh=document.getElementById('mmOpenHint');if(oh)oh.style.display='none';"
    +   "mmHideErr('mmMejaErr');mmHideErr('mmDurasiErr');mmHideErr('mmDetailMainErr');}"
    + "function mmResetItems(){var c=document.getElementById('mmMenuItems');if(c){c.innerHTML='';mmAddItem();}var sub=document.getElementById('mmKopiSub');if(sub){sub.style.display='none';sub.innerHTML='';}mmHideErr('mmKopiErr');}"
    + "function closeManual(){var ov=document.getElementById('mManual');if(ov)ov.classList.remove('open');}"
    + "function manualSetJenis(v){var h=document.getElementById('mmJenis');if(h)h.value=v;document.querySelectorAll('#mmJenisTog .mm-tog-btn').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-v')===v);});var sel=document.getElementById('mmKategori');if(sel)sel.innerHTML=(v==='pengeluaran')?MM_KAT_OUT:MM_KAT_IN;"
    +   "var actWrap=document.getElementById('mmActWrap');"
    +   "if(v==='pengeluaran'){if(actWrap)actWrap.style.display='none';"
    +     "var sewa=document.getElementById('mmSewaWrap');if(sewa)sewa.style.display='none';"
    +     "mmShowKopiWrap(false);"
    +     "var lain=document.getElementById('mmLainnyaWrap');if(lain)lain.style.display='';"
    +     "var jrow=document.getElementById('mmJumlahRow');if(jrow)jrow.style.display='';"
    +     "var jlbl=document.getElementById('mmJumlahLbl');if(jlbl)jlbl.textContent='Nominal';"
    +   "}else{if(actWrap)actWrap.style.display='';mmSetAct(mmAct);}"
    +   "manualInfo();}"
    + "function mmShowKopiWrap(show){var w=document.getElementById('mmKopiWrap');if(w)w.style.display=show?'':'none';var lbl=document.getElementById('mmKopiLbl');if(lbl)lbl.textContent=(mmAct==='sewa')?'Item Minum/Makan':'Item Pesanan';}"
    + "function mmSetAct(a){mmAct=a;document.querySelectorAll('#mmActTog .mm-tog-btn').forEach(function(b){b.classList.toggle('active',b.getAttribute('data-a')===a);});"
    +   "var sewa=document.getElementById('mmSewaWrap');if(sewa)sewa.style.display=(a==='sewa')?'':'none';"
    +   "var lain=document.getElementById('mmLainnyaWrap');if(lain)lain.style.display=(a==='lainnya')?'':'none';"
    +   "var addonOn=(document.getElementById('mmKopiAddonCb')||{}).checked;"
    +   "mmShowKopiWrap(a==='kopi'||(a==='sewa'&&addonOn));"
    +   "var jrow=document.getElementById('mmJumlahRow');if(jrow)jrow.style.display=(a==='kopi')?'none':'';"
    +   "var jlbl=document.getElementById('mmJumlahLbl');if(jlbl)jlbl.textContent=(a==='sewa')?'Nominal (otomatis dari tarif)':'Nominal';"
    +   "if(a==='kopi'){mmCalcAddon();}else if(a==='sewa'){mmComputeBilliardPrice();}"
    +   "manualInfo();}"
    + "function mmToggleAddon(cb){var on=!!cb.checked;var lbl=document.getElementById('mmKopiAddonLbl');if(lbl)lbl.classList.toggle('on',on);mmShowKopiWrap(mmAct==='kopi'||(mmAct==='sewa'&&on));mmCalcAddon();manualInfo();}"
    + "function mmDurasiChange(v){var el=document.getElementById('mmDetailMain');if(el)el.style.display=(v==='Open / Loss')?'':'none';mmHideErr('mmDetailMainErr');mmHideErr('mmDurasiErr');mmComputeBilliardPrice();}"
    + "function mmComputeBilliardPrice(){if(mmAct!=='sewa')return;var dr=document.getElementById('mmDurasi');var jEl=document.getElementById('mmJumlah');var mj=document.getElementById('mmMeja');if(!dr||!jEl)return;"
    +   "var opt=(mj&&mj.selectedIndex>0)?mj.options[mj.selectedIndex]:null;"
    +   "var ts=opt?parseInt(opt.dataset.ts||'0')||0:0;var tm=opt?parseInt(opt.dataset.tm||'0')||0:0;var to=opt?parseInt(opt.dataset.to||'0')||0:0;"
    +   "var dtv=(document.getElementById('mmDatetime')||{}).value||'';var hr=dtv?parseInt(dtv.slice(11,13))||0:0;var rate=(hr>=8&&hr<18)?ts:tm;"
    +   "var hint=document.getElementById('mmOpenHint');var v=dr.value;"
    +   "if(v==='Open / Loss'){jEl.value='';"
    +     "if(hint){if(to>0){hint.style.display='';hint.innerHTML='<i class=\"ti ti-info-circle\"></i> Acuan tarif Open: <b>Rp '+to.toLocaleString('id-ID')+'</b>/jam \\u2014 harga diisi manual.';}else{hint.style.display='none';}}"
    +     "manualInfo();return;}"
    +   "if(hint)hint.style.display='none';"
    +   "var m=v&&v.match(/^(\\d+) Jam$/);if(!m){manualInfo();return;}"
    +   "var jam=parseInt(m[1]);if(rate>0){jEl.value=String(jam*rate);manualRupiah(jEl);}"
    +   "manualInfo();}"
    ...
    +   "r.innerHTML='<div class=\"mip-wrap\"><select class=\"fsel mip-sel\" onchange=\"mmItemChange(this)\">'+WIZ_MENU_OPTS+'</select>'"
    +     "+'<button type=\"button\" class=\"mip-btn\" onclick=\"openItemPicker(this)\"><span class=\"mip-text\">Pilih item...</span><i class=\"ti ti-chevron-down\" style=\"font-size:14px;color:var(--txt3);flex-shrink:0\"></i></button></div>'"
    +     "+'<div class=\"fin-qty-stepper\"><button type=\"button\" class=\"fin-qty-btn\" onclick=\"mmQtyMinus(this)\" aria-label=\"Kurangi\">\\u2212</button>'"
    +     "+'<input type=\"number\" class=\"fin-qty-inp\" value=\"1\" min=\"1\" inputmode=\"numeric\" oninput=\"mmCalcAddon()\">'"
    +     "+'<button type=\"button\" class=\"fin-qty-btn\" onclick=\"mmQtyPlus(this)\" aria-label=\"Tambah\">+</button></div>'"
    +     "+'<button type=\"button\" class=\"fin-btn-rm-row\" onclick=\"mmRmItem(this)\"><i class=\"ti ti-x\"></i></button>'"
    +     "+MM_EXTRAS_HTML;"
    +   "c.appendChild(r);}"
    + "function mmItemChange(sel){var row=sel.closest('.fin-menu-row');var opt=sel.selectedIndex>0?sel.options[sel.selectedIndex]:null;var extras=row.querySelector('.wiz-extras');var tempDiv=row.querySelector('.wiz-temp');var topsDiv=row.querySelector('.wiz-tops');"
    +   "if(!opt||!opt.value){if(extras)extras.style.display='none';mmCalcAddon();return;}"
    +   "var hargaHot=parseInt(opt.dataset.hargaHot||'0');var kat=opt.dataset.kategori||'minuman';var nama=opt.value;var showTemp=kat==='minuman'&&hargaHot>0;var tops=WIZ_TOPPINGS[nama]||[];var showTops=tops.length>0;"
    +   "if(tempDiv){tempDiv.style.display=showTemp?'flex':'none';if(showTemp&&!row.dataset.temp)row.dataset.temp='ice';}"
    +   "if(topsDiv){topsDiv.style.display=showTops?'flex':'none';if(showTops){topsDiv.innerHTML=tops.map(function(t){return '<label style=\"display:flex;align-items:center;gap:6px;font-size:11px;color:var(--txt);cursor:pointer\"><input type=\"checkbox\" data-top-harga=\"'+t.harga+'\" onchange=\"mmCalcAddon()\" style=\"width:14px;height:14px;accent-color:#3a7d2c;cursor:pointer\"><span>'+t.nama+'</span><span style=\"color:var(--txt2)\">+Rp '+Number(t.harga).toLocaleString('id-ID')+'</span><input type=\"number\" value=\"1\" min=\"1\" max=\"10\" onchange=\"mmCalcAddon()\" style=\"width:44px;padding:2px 6px;border-radius:5px;border:1px solid var(--border2);font-size:11px;text-align:center;background:var(--surface2);color:var(--txt);font-family:inherit\"></label>';}).join('');}}"
    +   "if(extras)extras.style.display=(showTemp||showTops)?'flex':'none';mmCalcAddon();}"
    + "function mmSetTemp(btn,temp){var row=btn.closest('.fin-menu-row');row.dataset.temp=temp;var tempDiv=row.querySelector('.wiz-temp');if(tempDiv){tempDiv.querySelectorAll('button').forEach(function(b){var isIce=b.dataset.temp==='ice';if(b.dataset.temp===temp){b.style.borderColor=isIce?'#3b82f6':'#ef4444';b.style.background=isIce?'rgba(59,130,246,.12)':'rgba(239,68,68,.1)';b.style.color=isIce?'#3b82f6':'#ef4444';}else{b.style.borderColor='var(--border2)';b.style.background='transparent';b.style.color='var(--txt2)';}});}mmCalcAddon();}"
    + "function mmQtyMinus(btn){var inp=btn.parentElement.querySelector('.fin-qty-inp');var v=(parseInt(inp.value)||1)-1;inp.value=Math.max(1,v);mmCalcAddon();}"
    + "function mmQtyPlus(btn){var inp=btn.parentElement.querySelector('.fin-qty-inp');inp.value=(parseInt(inp.value)||0)+1;mmCalcAddon();}"
    + "function mmRmItem(btn){var row=btn.closest('.fin-menu-row');var rows=document.querySelectorAll('#mmMenuItems .fin-menu-row');"
    +   "if(rows.length>1){row.remove();mmCalcAddon();return;}"
    +   "var sel=row.querySelector('select');if(sel){sel.selectedIndex=0;sel.value='';}"
    +   "var disp=row.querySelector('.mip-text');if(disp){disp.textContent='Pilih item...';disp.classList.remove('mip-text-filled');}"
    +   "var qty=row.querySelector('.fin-qty-inp');if(qty)qty.value='1';"
    +   "var extras=row.querySelector('.wiz-extras');if(extras)extras.style.display='none';"
    +   "var tempDiv=row.querySelector('.wiz-temp');if(tempDiv)tempDiv.style.display='none';"
    +   "var topsDiv=row.querySelector('.wiz-tops');if(topsDiv){topsDiv.style.display='none';topsDiv.innerHTML='';}"
    +   "delete row.dataset.temp;mmCalcAddon();mmHideErr('mmKopiErr');}"
    + "function mmCalcAddon(){var arr=[];var sum=0;"
    +   "document.querySelectorAll('#mmMenuItems .fin-menu-row').forEach(function(r){var sel=r.querySelector('select');var qty=r.querySelector('.fin-qty-inp');if(!(sel&&sel.value&&sel.selectedIndex>0))return;"
    +     "var q=parseInt(qty?qty.value:'1')||1;var opt=sel.options[sel.selectedIndex];var kat=opt.dataset.kategori||'minuman';var hH=parseInt(opt.dataset.hargaHot||'0');var tmp=r.dataset.temp||'ice';"
    +     "var harga=(kat==='minuman'&&hH>0&&tmp==='hot')?hH:parseInt(opt.dataset.harga||'0')||0;sum+=harga*q;"
    +     "var iStr=q>1?q+'\\xD7 '+sel.value:sel.value;if(kat==='minuman'&&hH>0)iStr+=' ('+tmp+')';"
    +     "var ts=[];r.querySelectorAll('.wiz-tops label').forEach(function(lbl){var cb=lbl.querySelector('input[type=checkbox]');if(!cb||!cb.checked)return;var tq=parseInt(lbl.querySelector('input[type=number]')?lbl.querySelector('input[type=number]').value:'1')||1;var tn=lbl.querySelector('span')?lbl.querySelector('span').textContent:'';var topH=parseInt(cb.dataset.topHarga||'0');sum+=topH*tq;ts.push(tq>1?tq+'\\xD7 '+tn:tn);});"
    +     "if(ts.length)iStr+=' + '+ts.join(', ');arr.push(iStr);});"
    +   "var sub=document.getElementById('mmKopiSub');if(sub){if(sum>0){sub.style.display='';sub.innerHTML='<i class=\"ti ti-receipt\" style=\"font-size:13px;color:var(--green)\"></i> '+arr.length+' item \\u00b7 <b>Rp '+sum.toLocaleString('id-ID')+'</b>';}else{sub.style.display='none';sub.innerHTML='';}}"
    +   "if(mmAct==='kopi'){var jEl=document.getElementById('mmJumlah');if(jEl){jEl.value=sum>0?String(sum):'';manualRupiah(jEl);}}"
    +   "mmAddonTotal=sum;mmAddonKet=arr.join(', ');manualInfo();return {ket:mmAddonKet,total:sum};}"
    + "function manualSubmit(form){var dt=(document.getElementById('mmDatetime')||{}).value||'';var jenis=(document.getElementById('mmJenis')||{}).value||'pemasukan';"
    +   "if(!dt){alert('Pilih tanggal & jam transaksi.');return false;}"
    +   "if(new Date(dt).getTime()>Date.now()+60000){alert('Tanggal transaksi tidak boleh di masa depan.');return false;}"
    +   "var note=((document.getElementById('mmKet')||{}).value||'').trim();"
    +   "var katHidden=document.getElementById('mmKatHidden');var ketFinal=document.getElementById('mmKetFinal');var kopiKet=document.getElementById('mmKopiKet');var kopiJ=document.getElementById('mmKopiJ');var jEl=document.getElementById('mmJumlah');"
    +   "var amt=parseInt((jEl?jEl.value:'').replace(/\\D/g,''))||0;var kategori='';var ket='';"
    +   "if(kopiKet)kopiKet.value='';if(kopiJ)kopiJ.value='';"
    +   "if(jenis==='pengeluaran'){var ksel=document.getElementById('mmKategori');kategori=ksel?ksel.value:'';ket=note;if(!kategori){alert('Pilih kategori.');return false;}if(amt<=0){alert('Nominal harus lebih dari 0.');return false;}}"
    +   "else if(mmAct==='sewa'){var mj=document.getElementById('mmMeja');var dr=document.getElementById('mmDurasi');if(!mj||!mj.value){mmShowErr('mmMejaErr');return false;}if(!dr||!dr.value){mmShowErr('mmDurasiErr');return false;}kategori='Sewa Meja';ket=mj.value+' \\u00b7 '+dr.value;"
    +     "if(dr.value==='Open / Loss'){var dtl=document.getElementById('mmDetailMainInp');if(!dtl||!dtl.value.trim()){mmShowErr('mmDetailMainErr');return false;}ket+=' ('+dtl.value.trim()+')';}"
    +     "if(amt<=0){alert('Nominal harus lebih dari 0.');if(jEl)jEl.focus();return false;}"
    +     "var addonOn=(document.getElementById('mmKopiAddonCb')||{}).checked;if(addonOn){var fb=mmCalcAddon();if(!fb.total||fb.total<=0){mmShowErr('mmKopiErr');return false;}if(kopiKet)kopiKet.value=fb.ket;if(kopiJ)kopiJ.value=String(fb.total);}"
    +     "if(note)ket=ket+' \\u2014 '+note;}"
    +   "else if(mmAct==='kopi'){var fb2=mmCalcAddon();if(!fb2.total||fb2.total<=0){mmShowErr('mmKopiErr');return false;}kategori='Kopi / Snack';ket=fb2.ket;if(note)ket=ket+' \\u2014 '+note;amt=fb2.total;}"
    +   "else{var ksel2=document.getElementById('mmKategori');kategori=ksel2?ksel2.value:'';ket=note;if(!kategori){alert('Pilih kategori.');return false;}if(amt<=0){alert('Nominal harus lebih dari 0.');if(jEl)jEl.focus();return false;}}"
    +   "if(katHidden)katHidden.value=kategori;if(ketFinal)ketFinal.value=ket.slice(0,200);if(jEl)jEl.value=String(amt);"
    ...
    + "var wizS={step:1,tipe:'income',act:'kopi',waktu:'" + defaultShift + "',bayar:'cash',lunas:true,kopiAddon:false};"
    ...
    // Saat balik ke income, reset act ke 'kopi' (default) kalau sebelumnya 'other'.
    + "else if(wizS.act==='other'){wizSetAct('kopi');}"
    + "var dtEl=document.getElementById('wizDatetime');"
    ...
    + "wizS.act=a;"
    + "['kopi','other'].forEach(function(x){document.getElementById('wiz-'+x).className='fin-tog-btn'+(x===a?' sel':'');});}"
    ...
    // Auto-hitung harga Main Billiard dari TARIF PER MEJA (master Manajemen Meja).
    // Tarif diambil dari data-attr option meja terpilih: data-ts (siang) / data-tm
    // (malam) / data-to (open). Rate ikut sesi waktu (siang/malam). Durasi "N Jam"
    // → jumlah = jam × tarif. "Open / Loss" → field dikosongkan (manual) + tampilkan
    // tarif Open sbg acuan. Tanpa meja terpilih / tarif 0 → tidak menimpa field.
    + "function wizComputeBilliardPrice(){"
    + "if(wizS.act!=='billiard'||wizS.tipe!=='income')return;"
    + "var dr=document.getElementById('wizDurasi');var jEl=document.getElementById('wizJumlah');var mj=document.getElementById('wizMeja');"
    + "if(!dr||!jEl)return;"
    + "var opt=(mj&&mj.selectedIndex>0)?mj.options[mj.selectedIndex]:null;"
    + "var ts=opt?parseInt(opt.dataset.ts||'0')||0:0;var tm=opt?parseInt(opt.dataset.tm||'0')||0:0;var to=opt?parseInt(opt.dataset.to||'0')||0:0;"
    + "var rate=wizS.waktu==='malam'?tm:ts;"
    + "var hint=document.getElementById('wizOpenHint');var v=dr.value;"
    + "if(v==='Open / Loss'){jEl.value='';wizHideErr('wizJumlahErr');"
    + "if(hint){if(to>0){hint.style.display='';hint.innerHTML='<i class=\"ti ti-info-circle\"></i> Acuan tarif Open: <b>Rp '+to.toLocaleString('id-ID')+'</b>/jam — harga tetap diisi manual.';}else{hint.style.display='none';}}"
    + "wizRefreshAddonDisplay();return;}"
    + "if(hint)hint.style.display='none';"
    + "var m=v&&v.match(/^(\\d+) Jam$/);"
    + "if(!m){wizRefreshAddonDisplay();return;}"
    + "var jam=parseInt(m[1]);"
    + "if(rate>0){jEl.value=String(jam*rate);wizFmtJ(jEl);wizHideErr('wizJumlahErr');}"
    + "wizRefreshAddonDisplay();}"
    ...
    + "wizCalcTotal();}"
    // Klik X: kalau bukan row terakhir, hapus row. Kalau row terakhir → reset
    // (clear picker, qty=1, hide suhu/toppings). Bug sebelumnya: row terakhir
    // tidak respons sama sekali (silent no-op) karena guard length>1.
    + "function wizRmItem(btn){"
    + "var row=btn.closest('.fin-menu-row');"
    + "var rows=document.querySelectorAll('#wizMenuItems .fin-menu-row');"
    + "if(rows.length>1){row.remove();wizCalcTotal();return;}"
    // Reset row terakhir
    + "var sel=row.querySelector('select');"
    + "if(sel){sel.selectedIndex=0;sel.value='';}"
    + "var disp=row.querySelector('.mip-text');"
    + "if(disp){disp.textContent='Pilih item...';disp.classList.remove('mip-text-filled');}"
    + "var qty=row.querySelector('.fin-qty-inp');"
    + "if(qty)qty.value='1';"
    + "var extras=row.querySelector('.wiz-extras');"
    ...
    + "var topsDiv=row.querySelector('.wiz-tops');"
    + "if(topsDiv){topsDiv.style.display='none';topsDiv.innerHTML='';}"
    + "delete row.dataset.temp;"
    + "wizCalcTotal();"
    + "wizHideErr('wizKopiErr');"
    + "}"
...
export function financeMejaPage(role = "owner", mejaList = [], showErr = false, msg = "", occupiedMejaIds = [], openSesiByMeja = {}) {
  const toastMsg = msg === "added"   ? "Meja berhasil ditambah"
    : msg === "updated" ? "Perubahan meja tersimpan"
    : msg === "deleted" ? "Meja dihapus"
    : msg === "status"  ? "Status meja diperbarui"
    : msg === "massal"  ? "Tarif diterapkan ke meja terpilih"
    : "";
  const errHtml = showErr
  ...
  const cNonaktif = mejaList.filter((m) => m.status === "nonaktif").length;
  const cAktif    = cKosong + cDipakai;
  const okupansi  = cAktif > 0 ? Math.round((cDipakai / cAktif) * 100) : 0;
  ...
  const ballTxt = (m) => { const x = String(m.nama || "").match(/\d+/); return x ? x[0] : (String(m.nama || "?").trim().slice(0, 2) || "?"); };
  const tariffRows = (m) =>
    "<div class=\"mjc-trow\"><span class=\"mjc-tn\"><i class=\"ti ti-sun\"></i>Siang</span><span class=\"mjc-tv\">" + rp(m.tarif_siang || 0) + "<span class=\"u\">/jam</span></span></div>"
    + "<div class=\"mjc-trow\"><span class=\"mjc-tn\"><i class=\"ti ti-moon\"></i>Malam</span><span class=\"mjc-tv\">" + rp(m.tarif_malam || 0) + "<span class=\"u\">/jam</span></span></div>"
    + "<div class=\"mjc-trow\"><span class=\"mjc-tn\"><i class=\"ti ti-infinity\"></i>Open</span><span class=\"mjc-tv open\" title=\"ikut tarif Siang/Malam\">ikut S/M</span></div>";
  const liveBlockFor = (m) => {
  ...
  const cards = mejaList.length === 0
    ...
    : mejaList.map((m, i) => {
        ...
        return "<div class=\"mjc-card s-" + st.cls + (m.status !== "aktif" ? " dim" : "") + (m.jenis === "9ft" ? " j-9ft" : "") + "\" data-status=\"" + fkey + "\" data-name=\"" + escHtml(String(m.nama || "").toLowerCase()) + "\" style=\"animation-delay:" + (i * 40) + "ms\">"
          ...
          + liveBlockFor(m)
          + "<div class=\"mjc-tarif\">" + tariffRows(m) + "</div>"
          + "<div class=\"mjc-foot\">" + primary + quick + overflow + "</div>"
          + "</div>";
      }).join("");
  ...
  const extraCss = `
    ...
    .mjc-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:16px}
    .mjc-add{flex-shrink:0;height:40px;padding:0 16px;font-weight:700}
    ...
    /* ── Stat strip (one connected bordered container) ─── */
    .mjc-stats{display:grid;grid-template-columns:repeat(4,1fr);background:var(--surface);border:1px solid var(--border);border-radius:14px;margin-bottom:20px;overflow:hidden}
    .mjc-stat{padding:16px 20px;border-right:1px solid var(--border)}
    .mjc-stat:last-child{border-right:none}
    .mjc-stat-k{display:flex;align-items:center;gap:7px;font-size:12px;font-weight:600;color:var(--txt2);margin:0}
    .mjc-stat-k .dt{width:7px;height:7px;border-radius:50%;flex-shrink:0}
    .dt-total{background:var(--green)}.dt-empty{background:#94a3b8}.dt-used{background:var(--blue)}.dt-maint{background:var(--amber)}
    .mjc-stat-v{font-size:28px;font-weight:800;letter-spacing:-.02em;color:var(--txt);font-family:var(--ff-mono);line-height:1;margin:9px 0 3px}
    .mjc-stat-m{font-size:11.5px;color:var(--txt3);font-weight:500}
    .mjc-stat-m b{color:var(--txt2);font-family:var(--ff-mono)}
    .mjc-bar{height:4px;border-radius:4px;background:var(--surface2);margin-top:11px;overflow:hidden}
    .mjc-bar i{display:block;height:100%;border-radius:4px;background:var(--green)}

    /* ── Toolbar (search + tabs + view toggle) ─────────── */
    .mjc-toolbar{display:flex;align-items:center;gap:11px;flex-wrap:wrap;margin-bottom:16px}
    .mjc-search{position:relative;flex:0 0 230px}
    .mjc-search>i{position:absolute;left:12px;top:50%;transform:translateY(-50%);color:var(--txt3);font-size:15px}
    .mjc-search input{width:100%;background:var(--surface);border:1px solid var(--border2);border-radius:10px;padding:10px 12px 10px 35px;color:var(--txt);font-family:var(--ff);font-size:13px;outline:none;transition:border-color .15s,box-shadow .15s;box-sizing:border-box}
    .mjc-search input:focus{border-color:var(--accent);box-shadow:0 0 0 3px var(--green-bg)}
    .mjc-tabs{display:flex;gap:2px;flex-wrap:wrap}
    .mjc-tab{display:inline-flex;align-items:center;gap:6px;border:1px solid transparent;background:none;cursor:pointer;padding:8px 13px;border-radius:9px;font-size:12.5px;font-weight:600;color:var(--txt2);font-family:var(--ff);transition:.15s}
    .mjc-tab .c{font-size:10.5px;font-weight:700;color:var(--txt3)}
    .mjc-tab:hover{background:var(--surface);border-color:var(--border2)}
    .mjc-tab.active{background:var(--green-bg);color:var(--green-dark);border-color:#cce8b8}
    .mjc-tab.active .c{color:var(--green-dark)}
    .mjc-right{margin-left:auto;display:flex;gap:8px}
    .mjc-bulk{display:inline-flex;align-items:center;gap:7px;border:1px solid var(--border2);background:var(--surface);color:var(--txt2);padding:9px 14px;border-radius:10px;font-size:12.5px;font-weight:600;cursor:pointer;font-family:var(--ff);transition:.15s}
    .mjc-bulk:hover{border-color:var(--accent);color:var(--accent)}
    .mjc-bulk i{font-size:15px}
    .mjc-view{display:flex;border:1px solid var(--border2);border-radius:10px;overflow:hidden;background:var(--surface)}
    .mjc-vt{width:36px;height:36px;border:none;background:none;display:grid;place-items:center;cursor:pointer;color:var(--txt3)}
    .mjc-vt i{font-size:16px}
    .mjc-vt.active{background:var(--green-bg);color:var(--green-dark)}
    .mjc-vt+.mjc-vt{border-left:1px solid var(--border2)}

    /* ── Grid ──────────────────────────────────────────── */
    .mjc-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(262px,1fr));gap:16px;align-items:start}
    .mjc-grid.list{grid-template-columns:1fr}
    ...
    /* Dipakai → hijau lebih menonjol (border lebih tegas + glow + bayangan kuat). */
    .mjc-card.s-use{border-color:#86c563;box-shadow:0 0 0 1px rgba(124,178,86,.35),0 2px 5px rgba(45,102,36,.1),0 16px 32px -14px rgba(45,102,36,.5)}
    .mjc-card.s-use .mjc-top{background:#e4f1d6;border-bottom-color:#c4e1ab}
    /* Kosong → abu-abu netral (mundur ke belakang) biar yang Dipakai makin kontras. */
    .mjc-card.s-ok{border-color:#dde0dd}
    .mjc-card.s-ok .mjc-top{background:#f3f5f3;border-bottom-color:#e6e9e6}
    .mjc-card.s-ok .mjc-chip{background:#9aa39a;color:#fff;box-shadow:none}
    ...
    .mjc-card.s-use .mjc-chip{background:var(--green);color:#fff;box-shadow:0 3px 8px rgba(45,102,36,.3)}
    .mjc-meta{flex:1;min-width:0}
    .mjc-meta h3{font-size:16px;font-weight:800;letter-spacing:-.01em;color:var(--txt);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    ...
    .mjc-trow{display:flex;align-items:center;padding:9px 8px;font-size:12.5px}
    .mjc-trow+.mjc-trow{border-top:1px solid var(--border)}
    .mjc-tn{display:flex;align-items:center;gap:8px;color:var(--txt2);font-weight:600}
    .mjc-tn i{font-size:15px;color:var(--txt3)}
    .mjc-tv{margin-left:auto;font-weight:800;color:var(--txt);font-family:var(--ff-mono)}
    .mjc-tv .u{font-size:10px;font-weight:500;color:var(--txt3);font-family:var(--ff);margin-left:1px}
    .mjc-tv.open{font-size:11px;font-weight:700;color:var(--txt3);font-family:var(--ff)}

    /* ── Foot (actions) ────────────────────────────────── */
    .mjc-foot{display:flex;gap:8px;padding:13px 16px;border-top:1px solid var(--border);margin-top:auto}
    .mjc-act{flex:1;display:inline-flex;align-items:center;justify-content:center;gap:7px;border:none;border-radius:9px;padding:10px;font-size:12.5px;font-weight:700;cursor:pointer;font-family:var(--ff);text-decoration:none;transition:.15s}
    .mjc-act i{font-size:15px}
    .mjc-act.open{background:var(--accent);color:#fff}
    .mjc-act.open:hover{background:var(--green-dark)}
    .mjc-act.view{background:var(--surface2);color:var(--txt2)}
    .mjc-act.view:hover{background:#e7ece5;color:var(--green-dark)}
    .mjc-iconbtn{width:38px;flex:0 0 38px;border:1px solid var(--border2);background:var(--surface);border-radius:9px;display:grid;place-items:center;cursor:pointer;color:var(--txt3);transition:.15s}
    .mjc-iconbtn:hover{border-color:var(--accent);color:var(--accent)}
    .mjc-iconbtn i{font-size:16px}

    /* ── Overflow menu ─────────────────────────────────── */
    .mjc-menu-wrap{position:relative;display:inline-flex}
    .mjc-menu{position:absolute;right:0;bottom:calc(100% + 6px);background:var(--surface);border:1px solid var(--border);border-radius:10px;box-shadow:0 8px 26px rgba(26,35,24,.16);padding:5px;min-width:182px;display:none;z-index:30}
    ...
    /* ── List mode ─────────────────────────────────────── */
    .mjc-grid.list .mjc-card{flex-direction:row;align-items:stretch}
    .mjc-grid.list .mjc-top{flex:0 0 232px;border-right:1px solid var(--border);border-bottom:none;align-items:center}
    .mjc-grid.list .mjc-session{margin:0;flex:0 0 216px;border-radius:0;border:none;border-right:1px solid var(--border);display:flex;flex-direction:column;justify-content:center;background:var(--green-bg)}
    .mjc-grid.list .mjc-tarif{flex:1;display:flex;align-items:center;justify-content:space-around;gap:10px;padding:0 18px;border-right:1px solid var(--border)}
    .mjc-grid.list .mjc-trow{flex-direction:column;align-items:flex-start;gap:3px;border-top:none!important;padding:0}
    .mjc-grid.list .mjc-tv{margin-left:0}
    .mjc-grid.list .mjc-foot{flex:0 0 auto;border-top:none;align-items:center;margin-top:0}
    ...
    @media(max-width:680px){
      .mjc-stats{grid-template-columns:repeat(2,1fr)}
      .mjc-stat:nth-child(2n){border-right:none}
      .mjc-stat:nth-child(1),.mjc-stat:nth-child(2){border-bottom:1px solid var(--border)}
      .mjc-add{width:100%}
      .mjc-search{flex:1 1 100%}
      .mjc-right{margin-left:0;width:100%}
      .mj-form{grid-template-columns:1fr;padding:16px}
      .mjc-grid.list .mjc-card{flex-direction:column}
      .mjc-grid.list .mjc-top,.mjc-grid.list .mjc-session,.mjc-grid.list .mjc-tarif{flex:auto;border-right:none}
      .mjc-grid.list .mjc-top{border-bottom:1px solid var(--border)}
      .mjc-grid.list .mjc-session{border-bottom:1px solid var(--border)}
      .mjc-grid.list .mjc-tarif{flex-direction:row;justify-content:space-between;padding:4px 8px 8px;border-bottom:1px solid var(--border)}
      .mjc-grid.list .mjc-trow{flex-direction:row;padding:9px 8px}
      .mjc-grid.list .mjc-tv{margin-left:auto}
    }
  `;
  ...
  const js =
    ...
    + "var mjFilterKey='all';"
    + "function mjApply(){var qel=document.getElementById('mjQ');var q=(qel?qel.value:'').toLowerCase().trim();document.querySelectorAll('.mjc-card').forEach(function(c){var okF=mjFilterKey==='all'||c.dataset.status===mjFilterKey;var okQ=(c.dataset.name||'').indexOf(q)>=0;c.style.display=(okF&&okQ)?'':'none';});}"
    + "function mjPill(el){document.querySelectorAll('.mjc-tab').forEach(function(x){x.classList.remove('active');});el.classList.add('active');mjFilterKey=el.dataset.f;mjApply();}"
    ...
    // Tarif Open: ladder per bracket waktu [0-10,10-20,20-30,30-45,45-60] menit/jam,
    // <10mnt=Rp0 (bisa batal). Tiap bracket ikut jam dindingnya (split 18.00) & berulang
    // tiap jam (naik terus). Increment per bracket (kelipatan tarif):
    //   SIANG /10 -> [2,3,0,3,2]  (kumulatif 2,5,5,8,10 utk 7ft 10rb)
    //   MALAM /6  -> [1,1,1,1,2]  (kumulatif 2,4,6,8,12 utk 7ft 12rb)
    // 9ft skala ke tarifnya. Total akhir dibulatkan ke kelipatan Rp 1.000 (tak ada ratusan).
    + "function openTarif(startMs,endMs,si,ma){var s=Math.floor((endMs-startMs)/1000);if(s<600)return 0;var tMin=s/60;var bs=[0,10,20,30,45];var incS=[2,3,0,3,2],incM=[1,1,1,1,2];var tot=0;for(var g=0;;g++){var p=g%5,hr=Math.floor(g/5);var startG=hr*60+bs[p];if(g>0&&startG>=tMin)break;var bms=startMs+startG*60000;if(wibSiangAt(bms))tot+=incS[p]*si/10;else tot+=incM[p]*ma/6;}return Math.round(tot/1000)*1000;}"
  ...
  return docHeadV4("Manajemen Meja")
    ...
    + "<div class=\"mjc-stats\">"
    +   "<div class=\"mjc-stat\"><div class=\"mjc-stat-k\"><span class=\"dt dt-total\"></span>Total meja</div><div class=\"mjc-stat-v\">" + mejaList.length + "</div>"
    +     "<div class=\"mjc-stat-m\">Okupansi <b>" + okupansi + "%</b> · " + cKosong + " siap</div>"
    +     "<div class=\"mjc-bar\"><i style=\"width:" + okupansi + "%\"></i></div></div>"
    +   "<div class=\"mjc-stat\"><div class=\"mjc-stat-k\"><span class=\"dt dt-empty\"></span>Kosong</div><div class=\"mjc-stat-v\">" + cKosong + "</div><div class=\"mjc-stat-m\">Siap menerima sesi</div></div>"
    +   "<div class=\"mjc-stat\"><div class=\"mjc-stat-k\"><span class=\"dt dt-used\"></span>Dipakai</div><div class=\"mjc-stat-v\">" + cDipakai + "</div><div class=\"mjc-stat-m\">Estimasi berjalan <b id=\"mjLiveRev\">" + rp(0) + "</b></div></div>"
    +   "<div class=\"mjc-stat\"><div class=\"mjc-stat-k\"><span class=\"dt dt-maint\"></span>Maintenance</div><div class=\"mjc-stat-v\">" + cMaint + "</div><div class=\"mjc-stat-m\">" + (cMaint > 0 ? "Sedang perbaikan" : "Tidak ada perbaikan") + "</div></div>"
    + "</div>"
    + (mejaList.length > 0 ? "<div class=\"mjc-toolbar\">"
    +   "<div class=\"mjc-search\"><i class=\"ti ti-search\"></i><input id=\"mjQ\" type=\"text\" placeholder=\"Cari meja…\" oninput=\"mjApply()\"></div>"
    +   "<div class=\"mjc-tabs\" id=\"mjPills\">"
    +     "<button type=\"button\" class=\"mjc-tab active\" data-f=\"all\" onclick=\"mjPill(this)\">Semua <span class=\"c\">" + mejaList.length + "</span></button>"
    +     "<button type=\"button\" class=\"mjc-tab\" data-f=\"empty\" onclick=\"mjPill(this)\">Kosong <span class=\"c\">" + cKosong + "</span></button>"
    +     "<button type=\"button\" class=\"mjc-tab\" data-f=\"use\" onclick=\"mjPill(this)\">Dipakai <span class=\"c\">" + cDipakai + "</span></button>"
    +     "<button type=\"button\" class=\"mjc-tab\" data-f=\"mtc\" onclick=\"mjPill(this)\">Maintenance <span class=\"c\">" + cMaint + "</span></button>"
    ...
    + "</form></div></div>"
    + "<div class=\"mj-modal\" id=\"mjBulkModal\" onclick=\"if(event.target===this)closeBulk()\">"
    + "<div class=\"mj-modal-card\">"
    + "<div class=\"mj-modal-hd\"><span><i class=\"ti ti-adjustments\"></i> Terapkan Tarif ke Banyak Meja</span><button type=\"button\" class=\"mj-modal-x\" onclick=\"closeBulk()\"><i class=\"ti ti-x\"></i></button></div>"
    + "<form action=\"/operasional/meja/tarif-massal\" method=\"post\" class=\"mj-form mj-form-modal\" onsubmit=\"return confirm('Terapkan tarif ini ke meja terpilih? Tarif lama akan ditimpa.')\">"
    + "<div class=\"mj-fg mj-fg-name\"><label>Berlaku untuk</label><select name=\"jenis\" onchange=\"mjPreset(this)\"><option value=\"all\">Semua meja</option><option value=\"7ft\">Hanya 7 feet (bola tanggung)</option><option value=\"9ft\">Hanya 9 feet (bola besar)</option></select></div>"
    + tarifInput("tarif_siang", "Tarif Siang / jam") + tarifInput("tarif_malam", "Tarif Malam / jam")
...
// ── Halaman Sesi / Bill Meja ──────────────────────────────────
// Satu sesi = wadah transaksi utk satu meja (1 sewa + banyak F&B). Tiap item =
...
export function financeSesiPage({ role = "owner", displayName = "", sesiList = [], mejaTersedia = [], menuItems = [], msg = "", durBaru = "", shift = "siang" }) {
  const toastMsg = msg === "dibuka"  ? "Sesi meja dibuka"
    ...
    : msg === "batal"     ? "Sesi dibatalkan — meja kembali kosong, tidak ada yang tercatat"
  ...
  const _menuOpt = (m) =>
    "<option value=\"" + escHtml(m.nama) + "\" data-harga=\"" + (m.harga || 0) + "\" data-harga-hot=\"" + (m.harga_hot || 0) + "\" data-kategori=\"" + escHtml(m.kategori || "minuman") + "\">" + escHtml(m.nama) + " — " + rp(m.harga || 0) + "</option>";
  ...
  const payForm = (sesiId, itemId, amtStr, nama) => {
    ...
    return "<button type=\"button\" class=\"chip-btn\" onclick='openBayar(" + plC + ")'><i class=\"ti ti-cash\"></i> Cash</button>"
      + "<button type=\"button\" class=\"chip-btn\" onclick='openBayar(" + plQ + ")'><i class=\"ti ti-qrcode\"></i> QRIS</button>";
  };
  ...
  const renderFnbRow = (s, t) => {
    ...
    const sub    = paid
      ...
      : "<div class=\"line-sub unpaid\"><span class=\"d\"></span> Belum bayar</div>";
    const pay    = paid ? "" : payForm(s.id, t.id, rp(line), String(t.keterangan || t.kategori || "Item"));
    const delMsg = paid
    ...
    const del = "<form method=\"post\" action=\"/operasional/sesi/item/hapus\" style=\"display:contents\" onsubmit=\"return confirm('" + escHtml(delMsg) + "')\">"
      ...
      + "<button type=\"submit\" class=\"line-void\" title=\"Hapus item\"><i class=\"ti ti-trash\"></i></button></form>";
    return "<div class=\"line" + (paid ? " paid" : "") + "\"><div class=\"line-main\">"
      + "<div class=\"line-title\">" + escHtml(t.keterangan || t.kategori || "Item") + "</div>" + sub + "</div>"
      + "<div class=\"line-right\"><span class=\"line-amt\">" + rp(line) + "</span>" + pay + del + "</div></div>";
  };

  // Baris sewa meja: durasi + tombol Ubah durasi + status bayar saat tutup.
  const renderSewaRow = (s, sewa) => {
    if (!sewa) return "<div class=\"line line-none\">Sewa meja belum tercatat.</div>";
    ...
    const pl = escHtml(JSON.stringify({ sid: s.id, ts: s.tarif_siang || 0, tm: s.tarif_malam || 0, dur: durLabel, start: s.opened_at || "", w: sewa.waktu === "malam" ? "malam" : "siang", paid: sewa.lunas !== false, mtd: sewa.bayar === "qris" ? "qris" : "cash" }));
    ...
    if (_segs.length > 1 && startMs) {
      ...
      + "<form method=\"post\" action=\"/operasional/sesi/sewa/undo\" style=\"display:contents\" onsubmit=\"return confirm('Batalkan perpanjangan terakhir (+" + _segs[_segs.length - 1] + " jam)? Harga sewa dihitung ulang.')\">"
  ...
  const details = sesiData.length === 0
    ...
    : sesiData.map((d, i) => {
        ...
        const batalPayload  = escHtml(JSON.stringify({ id: s.id, meja: s.nama_meja || "Meja", paid: d.dibayar }));
        ...
        return "<div class=\"bsesi\" data-detail=\"" + s.id + "\"" + (i > 0 ? " style=\"display:none\"" : "") + ">"
          ...
          + "<div class=\"bhead-meta\"><h2>" + escHtml(s.nama_meja || "Meja") + "</h2>"
          ...
          + fnbHtml + "</div></div>"
          + "<div class=\"btotals\"><div class=\"btot\"><small>Total</small><b>" + rp(d.total) + "</b></div>"
          + "<div class=\"btot\"><small>Dibayar</small><b class=\"" + (d.dibayar > 0 ? "good" : "muted") + "\">" + rp(d.dibayar) + "</b></div>"
          ...
          + "<button type=\"button\" class=\"bact danger\" onclick='openBatal(" + batalPayload + ")'><i class=\"ti ti-circle-x\"></i> Batal Sesi</button>"
  ...
  const extraCss = `
    /* ── Page header ───────────────────────────────────── */
    .sesi-back{display:inline-flex;align-items:center;gap:5px;font-size:12px;color:var(--accent);text-decoration:none;font-weight:600;margin-bottom:14px}
    .sesi-back:hover{color:var(--green-dark)}
    .sesi-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:16px}
    .sesi-add{flex-shrink:0;height:40px;padding:0 16px;font-weight:700}
    .sesi-note{display:flex;gap:9px;align-items:flex-start;background:var(--green-bg);border:1px solid rgba(58,125,44,.22);border-radius:12px;padding:12px 15px;font-size:12.5px;color:var(--txt2);line-height:1.55;margin-bottom:18px}
    .sesi-note i{color:var(--accent);font-size:16px;flex-shrink:0;margin-top:1px}
    .sesi-note b{color:var(--txt);font-weight:700}
    .sesi-alert{display:flex;align-items:center;gap:8px;background:#fef6e7;color:#b45309;border:1px solid #f6dca6;border-radius:10px;padding:10px 12px;font-size:13px;margin-bottom:16px}

    /* ── Master-detail layout ──────────────────────────── */
    .bsesi-layout{display:grid;grid-template-columns:260px 1fr;gap:16px;align-items:start}
    ...
    .brail-item{display:block;width:100%;text-align:left;border:1px solid var(--border2);background:var(--surface);border-radius:12px;padding:13px;cursor:pointer;transition:.15s;font-family:var(--ff);color:var(--txt)}
    .brail-item:hover{border-color:#d4ddd7;background:var(--bg)}
    .brail-item.active{border:1.5px solid var(--green);background:var(--green-bg)}
    .bri-top{display:flex;align-items:center;gap:10px}
    .bri-num{width:28px;height:28px;border-radius:8px;background:var(--green);color:#fff;display:grid;place-items:center;font-family:var(--ff-mono);font-weight:800;font-size:12.5px;flex:0 0 auto}
    .bri-nm{font-size:13.5px;font-weight:800;color:var(--txt);flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    ...
    .bri-amt{font-family:var(--ff-mono);font-weight:800;font-size:13.5px;color:var(--txt)}
    ...
    .bsesi{background:var(--surface);border:1px solid var(--border2);border-radius:14px;overflow:hidden;box-shadow:0 1px 2px rgba(20,40,28,.04),0 10px 26px -14px rgba(20,40,28,.18)}
    .bhead{display:flex;align-items:center;gap:14px;padding:20px 24px;border-bottom:1px solid var(--line)}
    .bnum{width:46px;height:46px;border-radius:13px;flex:0 0 auto;display:grid;place-items:center;font-family:var(--ff-mono);font-size:18px;font-weight:800;background:var(--green);color:#fff;box-shadow:0 3px 8px rgba(45,102,36,.3)}
    .bhead-meta{flex:1;min-width:0}
    .bhead-meta h2{font-size:19px;font-weight:800;letter-spacing:-.02em;color:var(--txt)}
    .bhead-sub{font-size:12px;color:var(--txt3);margin-top:3px;font-weight:500}
    .bhead-sub b{color:var(--txt2);font-weight:700}
    .bhead-r{text-align:right;flex:0 0 auto}
    .bbadge{display:inline-flex;align-items:center;gap:5px;font-size:9px;font-weight:800;letter-spacing:.07em;text-transform:uppercase;background:var(--green-bg);color:var(--green-dark);border:1px solid #cce8b8;padding:4px 9px;border-radius:7px}
    ...
    .sec-label{font-size:9.5px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--txt3)}

    .line{display:flex;align-items:flex-start;gap:16px}
    .line+.line{margin-top:14px;padding-top:14px;border-top:1px solid var(--line)}
    .line.paid{opacity:.7}
    .line-none{color:var(--txt3);font-size:12.5px;font-style:italic;padding:4px 0}
    .line-main{flex:1;min-width:0}
    .line-title{display:flex;align-items:center;gap:8px;font-size:14px;font-weight:700;color:var(--txt)}
    .line-title i{font-size:15px;color:var(--muted-2);flex:0 0 auto}
    .line-sub{font-size:11.5px;color:var(--txt3);margin-top:4px;font-weight:500}
    .line-sub.paid{display:inline-flex;align-items:center;gap:5px;color:var(--green);font-weight:700}
    .line-sub.unpaid{display:inline-flex;align-items:center;gap:5px;color:#b45309;font-weight:700}
    .line-sub.unpaid .d{width:5px;height:5px;border-radius:50%;background:#d97706}
    ...
    .rinci-undo{align-self:flex-start;margin-top:2px;background:none;border:none;padding:2px 0;color:var(--txt3);font-size:11px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:4px;transition:.15s}
    .rinci-undo:hover{color:#b45309}
    ...
    /* totals strip (connected) */
    .btotals{display:grid;grid-template-columns:repeat(3,1fr);margin:8px 24px 0;border:1px solid var(--border2);border-radius:12px;overflow:hidden}
    .btot{padding:15px 18px;border-right:1px solid var(--line)}
    .btot:last-child{border-right:none}
    .btot small{font-size:9.5px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--txt3);display:block;margin-bottom:7px}
    .btot b{font-size:22px;font-weight:800;font-family:var(--ff-mono);letter-spacing:-.02em;color:var(--txt)}
    .btot b.good{color:var(--green-dark)}
    .btot b.muted{color:var(--muted-2)}
    .btot.due{background:var(--green-bg)}
    .btot.due b{color:var(--green-dark)}

    .bhint{font-size:11.5px;color:var(--txt3);padding:14px 24px 4px;line-height:1.5;display:flex;align-items:flex-start;gap:7px}
    .bhint i{flex:0 0 auto;margin-top:2px;color:var(--muted-2)}
    .bfoot{display:flex;gap:10px;padding:18px 24px 22px;flex-wrap:wrap}
    .bact{display:inline-flex;align-items:center;justify-content:center;gap:8px;border:1px solid var(--border2);background:var(--surface);color:var(--txt2);padding:12px 18px;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;font-family:var(--ff);transition:.15s}
    ...
    .bact.danger{color:#b91c1c;border-color:#f1c7c7}
    .bact.danger:hover{border-color:#dc2626;color:#dc2626;background:#fef2f2}
    ...
    .sesi-tot{flex:1;display:flex;flex-direction:column;gap:2px}
    .sesi-tot span{font-size:10px;color:var(--txt3);text-transform:uppercase;letter-spacing:.04em;font-weight:700}
    .sesi-tot b{font-size:14px;font-weight:700;color:var(--txt);font-family:var(--ff-mono)}
    .sesi-tot.ok b{color:var(--green)}
    .sesi-tot.sisa b{color:#b3791b}
    ...
    /* ── Modal + form (shared style) ───────────────────── */
    .mj-fg{display:flex;flex-direction:column;gap:5px;min-width:0}
    .mj-fg label{font-size:11px;font-weight:600;color:var(--txt3)}
    .mj-fg>input,.mj-fg>select{width:100%;padding:9px 11px;border:1px solid var(--border2);border-radius:8px;font-size:13px;font-family:var(--ff);color:var(--txt);background:var(--surface2);outline:none;box-sizing:border-box}
    .mj-fg>input:focus,.mj-fg>select:focus{border-color:var(--accent);background:var(--surface)}
    ...
    .mj-tog input{display:none}
    .mj-tog input:checked+span{color:var(--green-dark)}
    ...
    @media(max-width:680px){.sesi-add{width:100%}}
  `;
  ...
  const js =
    ...
    + "function bukaCalc(){var ms=document.getElementById('bukaMeja');if(!ms)return;var o=ms.options[ms.selectedIndex];var dur=document.getElementById('bukaDurasi').value;var el=document.getElementById('bukaPrice');var txt='—';if(o&&o.value){if(dur==='Open')txt='Diisi saat tutup';else{var m=dur.match(/^(\\d+) Jam$/);if(m){var jam=parseInt(m[1]);var si=parseInt(o.dataset.ts||'0')||0,ma=parseInt(o.dataset.tm||'0')||0;var st=bukaStartMs();txt='Rp '+tarifSplit(st,st+jam*3600000,si,ma).toLocaleString('id-ID');}}}el.textContent=txt;bukaTotal();}"
    + "function bukaTotal(){var el=document.getElementById('bukaGrandTotal');if(!el)return;var pe=document.getElementById('bukaPrice');var sewa=pe?(parseInt((pe.textContent||'').replace(/\\D/g,''))||0):0;var fe=document.querySelector('#mBuka .fnb-total');var fnb=fe?(parseInt((fe.textContent||'').replace(/\\D/g,''))||0):0;el.textContent='Rp '+(sewa+fnb).toLocaleString('id-ID');var dur=document.getElementById('bukaDurasi');var h=document.getElementById('bukaTotalHint');if(h)h.textContent=(dur&&dur.value==='Open')?'+ sewa dihitung saat selesai':'';}"
    + "function openUbahDurasi(d){document.getElementById('durSesiId').value=d.sid;window._durTs=d.ts||0;window._durTm=d.tm||0;window._durStart=d.start?new Date(d.start).getTime():Date.now();var m=String(d.dur||'').match(/^(\\d+) Jam$/);window._durBase=m?parseInt(m[1]):0;window._durHours=(window._durBase||0)+1;if(window._durHours<1)window._durHours=1;if(window._durHours>12)window._durHours=12;var ne=window._durBase?(' · selesai '+new Date(window._durStart+window._durBase*3600000).toLocaleTimeString('id-ID',{hour:'2-digit',minute:'2-digit'})):'';document.getElementById('durNowLbl').textContent=(window._durBase?(window._durBase+' Jam'):'Open')+ne;var bs=d.paid?'1':'0';var br=document.querySelector('#mDurasi input[name=\"bayar_status\"][value=\"'+bs+'\"]');if(br)br.checked=true;var mr=document.querySelector('#mDurasi input[name=\"metode\"][value=\"'+(d.mtd==='qris'?'qris':'cash')+'\"]');if(mr)mr.checked=true;durBayarToggle();durCalc();_show('mDurasi');}function closeDurasi(){_hide('mDurasi');}"
    + "function durBayarToggle(){var s=document.querySelector('#mDurasi input[name=\"bayar_status\"]:checked');var w=document.getElementById('durMetodeWrap');if(w)w.style.display=(s&&s.value==='1')?'':'none';}"
    + "function durAdd(n){window._durHours=(window._durBase||0)+n;if(window._durHours<1)window._durHours=1;if(window._durHours>12)window._durHours=12;durCalc();}"
    + "function durStep(delta){var v=(window._durHours||1)+delta;if(v<1)v=1;if(v>12)v=12;window._durHours=v;durCalc();}"
    + "function durCalc(){var jam=window._durHours||1,base=window._durBase||0,st=window._durStart||Date.now(),ts=window._durTs||0,tm=window._durTm||0;document.getElementById('durHours').value=jam;document.getElementById('durValue').value=jam+' Jam';var diff=jam-base;var dl=document.getElementById('durDelta');dl.className='dur-delta'+(diff>0?' up':(diff<0?' down':''));dl.textContent=base===0?('set '+jam+' jam'):(diff>0?('+'+diff+' jam'):(diff<0?(diff+' jam'):'tetap'));var endTxt=new Date(st+jam*3600000).toLocaleTimeString('id-ID',{hour:'2-digit',minute:'2-digit'});var prev=document.getElementById('durPreview');if(prev)prev.innerHTML=(diff>0?('Perpanjang <b>+'+diff+' jam</b> \\u2192 '):'')+'total <b>'+jam+' Jam</b> \\u00b7 sampai <b>'+endTxt+'</b>';var added=tarifSplit(st,st+jam*3600000,ts,tm)-tarifSplit(st,st+base*3600000,ts,tm);var ae=document.getElementById('durAddAmt');if(ae){ae.textContent=(added>=0?'Tambahan Rp ':'Kurang Rp ')+Math.abs(added).toLocaleString('id-ID');ae.style.color=added<0?'#b45309':'var(--green-dark)';}}"
    ...
    + "function _fnbHarga(o,r){if(!o)return 0;var hot=parseInt(o.dataset.hargaHot||'0');var kat=o.dataset.kategori||'minuman';if(kat==='minuman'&&hot>0&&r&&r.dataset.temp==='panas')return hot;return parseInt(o.dataset.harga||'0')||0;}"
    + "function fnbCalc(elInScope){var scope=fnbBlock(elInScope);if(!scope)return;var tot=0;scope.querySelectorAll('.fnb-row').forEach(function(r){var s=r.querySelector('.fnb-sel');var o=s.options[s.selectedIndex];var h=(o&&s.value)?_fnbHarga(o,r):0;var q=parseInt(r.querySelector('.fnb-qty').value)||1;tot+=h*q;});var el=scope.querySelector('.fnb-total');if(el)el.textContent='Rp '+tot.toLocaleString('id-ID');if(scope.closest('#mBuka'))bukaTotal();}"
    + "function fnbItemChange(sel){var row=sel.closest('.fnb-row');var o=sel.value?sel.options[sel.selectedIndex]:null;var t=row?row.querySelector('.fnb-temp'):null;if(t){var hot=o?parseInt(o.dataset.hargaHot||'0'):0;var kat=o?(o.dataset.kategori||'minuman'):'';var show=!!(o&&kat==='minuman'&&hot>0);t.style.display=show?'flex':'none';if(show){if(!row.dataset.temp)row.dataset.temp='dingin';t.querySelectorAll('.fnb-temp-btn').forEach(function(b){b.classList.remove('is-cold','is-hot');if(b.dataset.temp===row.dataset.temp)b.classList.add(row.dataset.temp==='panas'?'is-hot':'is-cold');});}else{delete row.dataset.temp;}}fnbCalc(sel);}"
    + "function fnbSetTemp(btn,temp){var row=btn.closest('.fnb-row');if(!row)return;row.dataset.temp=temp;row.querySelectorAll('.fnb-temp-btn').forEach(function(b){b.classList.remove('is-cold','is-hot');if(b.dataset.temp===temp)b.classList.add(temp==='panas'?'is-hot':'is-cold');});fnbCalc(btn);}"
    + "function fnbSerialize(scope){var items=[];if(!scope)return items;scope.querySelectorAll('.fnb-row').forEach(function(r){var s=r.querySelector('.fnb-sel');if(!s.value)return;var o=s.options[s.selectedIndex];var hot=parseInt(o.dataset.hargaHot||'0');var kat=o.dataset.kategori||'minuman';var isVar=kat==='minuman'&&hot>0;var temp=r.dataset.temp||'dingin';var nama=s.value+(isVar?(' ('+temp+')'):'');items.push({nama:nama,qty:parseInt(r.querySelector('.fnb-qty').value)||1,harga:_fnbHarga(o,r)});});var inp=scope.querySelector('.fnb-items');if(inp)inp.value=JSON.stringify(items);return items;}"
    ...
    + "var sewaInp=document.querySelector('#tutSewaWrap input[name=sewa_harga]');if(sewaInp){var amt=(d.sewaJumlah>0)?d.sewaJumlah:openTarif(d.start?new Date(d.start).getTime():Date.now(),Date.now(),d.ts||0,d.tm||0);sewaInp.value=amt>0?Number(amt).toLocaleString('id-ID'):'';}"
    + "var hint=document.getElementById('tutOpenHint');if(hint){hint.textContent=((d.sewaJumlah||0)===0)?('Open dihitung per blok 10 menit, dibulatkan ke atas (Siang Rp '+Number(d.ts||0).toLocaleString('id-ID')+' / Malam Rp '+Number(d.tm||0).toLocaleString('id-ID')+' per jam). Di bawah 10 menit belum ditagih (bisa dibatalkan). Terisi otomatis, bisa diedit.'):'';}"
    ...
    + "function openBatal(d){var paid=d.paid||0;var msg=paid>0?('Batalkan sesi '+d.meja+'?\\n\\nSudah ada pembayaran Rp '+Number(paid).toLocaleString('id-ID')+' yang akan DIHAPUS dari pemasukan. Meja kembali kosong & tidak ada yang tercatat.\\n\\nLanjutkan?'):('Batalkan sesi '+d.meja+'?\\n\\nSesi dianggap tidak jadi \\u2014 semua item dihapus, meja kembali kosong, dan tidak ada yang tercatat di Riwayat.\\n\\nLanjutkan?');if(confirm(msg)){document.getElementById('batalSesiId').value=d.id;document.getElementById('batalForm').submit();}}"
  ...
  return docHeadV4("Sesi Meja")
    ...
    + "</head><body>"
    + "<div class=\"layout\">"
    + buildFinanceSidebar("", "sesi", role)
    + "<div class=\"main-wrap\">"
    + "<header class=\"topbar\">"
    + (role === "owner" ? buildOwnerMenuToggle() : "")
    + "<div class=\"topbar-brand\"><div class=\"sb-brand-icon\" style=\"width:28px;height:28px;font-size:14px;margin-right:6px\"><i class=\"ti ti-receipt\"></i></div>"
    + "<div><div class=\"topbar-name\">Sesi Meja</div><div class=\"topbar-label\">Billiard</div></div></div>"
    + "<div class=\"topbar-right\">" + buildFinanceTopbarProfile(role, displayName) + "</div></header>"
    + "<div class=\"page\">"
    + "<a href=\"/operasional\" class=\"sesi-back\"><i class=\"ti ti-arrow-left\" style=\"font-size:14px\"></i> Kembali ke Keuangan</a>"
    + "<div class=\"sesi-head\">"
    +   "<div><div class=\"page-title\">Sesi Meja (Bill)</div>"
    +   "<div class=\"page-sub\">Buka sesi per meja, tambah pesanan bertahap, tandai bayar per item, lalu tutup.</div></div>"
    +   "<button type=\"button\" class=\"btn-primary sesi-add\" onclick=\"openBuka()\"><i class=\"ti ti-plus\"></i> Buka Sesi</button>"
    + "</div>"
    ...
    +   (mejaTersedia.length === 0
        ...
        : "<form method=\"post\" action=\"/operasional/sesi/buka\" class=\"mj-form buka-form\" onsubmit=\"return bukaSubmit(this)\">"
          ...
          + "</div></div>"
          + "<div class=\"mj-fg\" id=\"bukaMetodeWrap\"><label>Metode bayar</label><div class=\"mj-tog\">"
    ...
    +   "<div class=\"mj-modal-hd\"><span><i class=\"ti ti-clock-edit\"></i> Perpanjang Sewa</span><button type=\"button\" class=\"mj-modal-x\" onclick=\"closeDurasi()\"><i class=\"ti ti-x\"></i></button></div>"
    ...
    +     "<div class=\"dur-now\"><i class=\"ti ti-clock\"></i> Sekarang: <b id=\"durNowLbl\">—</b></div>"
    +     "<div class=\"mj-fg\"><label>Perpanjang cepat</label><div class=\"mj-tog\">"
    +       "<button type=\"button\" class=\"chip-btn\" onclick=\"durAdd(1)\"><i class=\"ti ti-plus\"></i> 1 jam</button>"
    +       "<button type=\"button\" class=\"chip-btn\" onclick=\"durAdd(2)\"><i class=\"ti ti-plus\"></i> 2 jam</button>"
    +     "</div></div>"
    +     "<div class=\"mj-fg\"><label>Atau atur total durasi</label><div class=\"dur-set\"><div class=\"fnb-stepper dur-stepper\">"
    ...
    +     "</div><span class=\"dur-unit\">Jam total</span><span class=\"dur-delta\" id=\"durDelta\"></span></div></div>"
    +     "<div style=\"background:var(--green-bg);border-radius:8px;padding:10px 14px\"><div id=\"durPreview\" style=\"font-size:12.5px;color:var(--txt2)\">—</div><b id=\"durAddAmt\" style=\"font-size:17px;color:var(--green-dark);font-family:var(--ff-mono);display:block;margin-top:3px\">—</b></div>"
    +     "<div class=\"mj-fg\"><label>Status bayar sewa</label><div class=\"mj-tog\">"
    +       "<label><input type=\"radio\" name=\"bayar_status\" value=\"1\" onchange=\"durBayarToggle()\"><span><i class=\"ti ti-cash\"></i> Sudah dibayar</span></label>"
    +       "<label><input type=\"radio\" name=\"bayar_status\" value=\"0\" onchange=\"durBayarToggle()\"><span><i class=\"ti ti-clock\"></i> Bayar saat tutup</span></label>"
    +     "</div></div>"
    +     "<div class=\"mj-fg\" id=\"durMetodeWrap\"><label>Metode bayar</label><div class=\"mj-tog\">"
    +       "<label><input type=\"radio\" name=\"metode\" value=\"cash\" checked><span><i class=\"ti ti-cash\"></i> Cash</span></label>"
    +       "<label><input type=\"radio\" name=\"metode\" value=\"qris\"><span><i class=\"ti ti-qrcode\"></i> QRIS</span></label>"
    +     "</div></div>"
    +     "<div class=\"sesi-note\" style=\"margin:0\"><i class=\"ti ti-info-circle\"></i> <span>Cek <b>tambahan</b> &amp; <b>jam selesai</b>, pilih <b>status bayar</b>, lalu Konfirmasi. <b>Sudah dibayar</b> = sewa (termasuk tambahan) lunas; <b>Bayar saat tutup</b> = ditagih saat sesi ditutup.</span></div>"
    +     "<button type=\"submit\" class=\"btn-primary mj-submit\"><i class=\"ti ti-check\"></i> Konfirmasi Perpanjang</button>"
    +   "</form></div></div>"
    // Modal Tambah F&B
    ...
    +   "</form>"
    +   "<div id=\"fnbTpl\" style=\"display:none\"><div class=\"fnb-row\"><div class=\"mip-wrap\"><select class=\"fnb-sel mip-sel\" onchange=\"fnbItemChange(this)\"><option value=\"\">— pilih item —</option>" + menuOpts + "</select><button type=\"button\" class=\"mip-btn\" onclick=\"openItemPicker(this,'Pilih Item')\"><span class=\"mip-text\">pilih item</span><i class=\"ti ti-chevron-down\" style=\"font-size:14px;color:var(--txt3);flex-shrink:0\"></i></button></div><div class=\"fnb-stepper\"><button type=\"button\" class=\"fnb-step\" onclick=\"fnbStep(this,-1)\" aria-label=\"Kurangi jumlah\"><i class=\"ti ti-minus\"></i></button><input type=\"number\" class=\"fnb-qty\" value=\"1\" min=\"1\" inputmode=\"numeric\" oninput=\"fnbCalc(this)\"><button type=\"button\" class=\"fnb-step\" onclick=\"fnbStep(this,1)\" aria-label=\"Tambah jumlah\"><i class=\"ti ti-plus\"></i></button></div><button type=\"button\" class=\"fnb-rm\" onclick=\"fnbRmRow(this)\" aria-label=\"Hapus\"><i class=\"ti ti-x\"></i></button><div class=\"fnb-temp\" style=\"display:none\"><span class=\"fnb-temp-lbl\">Suhu:</span><button type=\"button\" class=\"fnb-temp-btn is-cold\" data-temp=\"dingin\" onclick=\"fnbSetTemp(this,'dingin')\">❄ Dingin</button><button type=\"button\" class=\"fnb-temp-btn\" data-temp=\"panas\" onclick=\"fnbSetTemp(this,'panas')\">☕ Panas</button></div></div></div>"
    + "</div></div>"
    ...
    +   "<div class=\"mj-modal-hd\"><span><i class=\"ti ti-receipt\"></i> Tutup Sesi — <span id=\"tutMeja\"></span></span><button type=\"button\" class=\"mj-modal-x\" onclick=\"closeTutup()\"><i class=\"ti ti-x\"></i></button></div>"
    +   "<form method=\"post\" action=\"/operasional/sesi/tutup\" class=\"mj-form\">"
    ...
    +         "<label><input type=\"radio\" name=\"sewa_bayar\" value=\"qris\"><span><i class=\"ti ti-qrcode\"></i> QRIS</span></label>"
    +       "</div></div>"
    +     "</div>"
    ...
    + "<form method=\"post\" action=\"/operasional/sesi/batal\" id=\"batalForm\" style=\"display:none\"><input type=\"hidden\" name=\"sesi_id\" id=\"batalSesiId\"></form>"
    + "<script>" + js + "</script>"
    + buildFinanceBottomNav(role)
    + "</body></html>";
}
...
export function financeMenuPage(role = "owner", items = [], toppings = [], hasErr = false, editItem = null, extras = {}) {
  ...
  const resepEditorBlock = (() => {
    ...
    const renderResepRow = (r, idx) => {
      ...
      return "<tr class=\"mp-rs-trow\" data-idx=\"" + idx + "\">"
        ...
        + "</select></td>"
        + "<td style=\"width:130px\"><input type=\"number\" name=\"qty\" class=\"mp-rs-qty\" step=\"1\" min=\"0\" value=\"" + qty + "\" oninput=\"rsRecompute()\" placeholder=\"0\"></td>"
  ...
  const js = ""
    ...
    // Resep editor JS (simple, no IIFE, plain function declarations)
    + "function rpFmt(n){n=Math.round(n||0);var sign=n<0?'-':'';n=Math.abs(n);return sign+'Rp '+String(n).replace(/\\B(?=(\\d{3})+(?!\\d))/g,'.');}"
    + "function rsRecompute(){"
    +   "var rows=document.querySelectorAll('#resepRows .mp-rs-trow');"
    +   "var total=0;"
    +   "rows.forEach(function(row){"
    +     "var sel=row.querySelector('select.mp-rs-bahan');"
    +     "var qin=row.querySelector('input.mp-rs-qty');"
    +     "var sub=row.querySelector('.mp-rs-sub-val');"
    +     "var sat=row.querySelector('.mp-rs-satuan');"
    +     "if(!sel||!qin)return;"
    +     "var opt=sel.options[sel.selectedIndex];"
    +     "var hp=(opt&&opt.dataset)?(parseInt(opt.dataset.hpporsi||0)||0):0;"
    +     "var label=(opt&&opt.dataset)?(opt.dataset.label||''):'';"
    +     "var qv=(qin.value||'').toString().replace(',','.');"
    +     "var q=parseFloat(qv)||0;"
    +     "var s=Math.round(hp*q);"
    +     "if(sub){sub.textContent=rpFmt(s);sub.setAttribute('data-sub',s);}"
    +     "if(sat)sat.textContent=label||'\\u2014';"
    +     "total+=s;"
    +   "});"
    +   "var hrgEl=document.getElementById('rsHarga');"
    +   "var harga=hrgEl?(parseInt(hrgEl.value)||0):0;"
    +   "var margin=harga-total;"
    +   "var pct=harga>0?Math.round((margin/harga)*1000)/10:0;"
    ...
    +     "var row=el.parentNode;while(row&&row.tagName!=='TR'){row=row.parentNode;}"
    +     "if(row){var qin=row.querySelector('input.mp-rs-qty');if(qin&&(!qin.value||parseFloat(qin.value)===0)){qin.value='1';}}"
    +   "}"
    +   "rsRecompute();"
    + "}"
    + "function rsAddRow(){"
    ...
    +   "var s=clone.querySelector('select.mp-rs-bahan');if(s)s.value='';"
    +   "var q=clone.querySelector('input.mp-rs-qty');if(q)q.value='';"
    +   "var sub=clone.querySelector('.mp-rs-sub-val');if(sub){sub.textContent='Rp 0';sub.setAttribute('data-sub','0');}"
    +   "var sat=clone.querySelector('.mp-rs-satuan');if(sat)sat.textContent='\\u2014';"
    +   "tbody.appendChild(clone);"
    + "}"
    + "function rsDelRow(btn){"
    +   "var row=btn.parentNode;while(row&&row.tagName!=='TR'){row=row.parentNode;}"
    +   "var tbody=document.getElementById('resepRows');"
    +   "if(!row||!tbody)return;"
    +   "if(tbody.querySelectorAll('.mp-rs-trow').length>1){row.parentNode.removeChild(row);}"
    +   "else{var s=row.querySelector('select');var q=row.querySelector('input');if(s)s.value='';if(q)q.value='';}"
    +   "rsRecompute();"
    + "}"
    + "if(document.getElementById('resepEditor')){rsRecompute();}"
    // Detail modal & history fetch — DIHAPUS sementara karena JS-nya bikin
    // syntax error yang nge-break recompute. Data tetep di DB (harga dus,
    // renteng, supplier, catatan, history) dan bisa di-edit via Edit form.
    + "function bbShowDetail(id){window.location.href='/operasional/menu?tab=bahan&editbahan='+id;}"
    + "function bbCloseDetail(){}"
