...
import { readDB, writeDB, appendLog, findMemberIndex } from "../utils/db.js";
...
router.get("/scan", (req, res) => {
  ...
  const member = members.find((m) => m.kode === kode);
...
router.post("/checkin", async (req, res) => {
  ...
  // Case 3: 10x → gratis + reset
  if (m.totalMain >= CONFIG.BATAS_MAIN) {
    m.totalGratis  = (m.totalGratis ?? 0) + 1;
    m.status       = "GRATIS";
    m.tanggalMulai = today.toISOString();
    const { totalGratis, totalMain: tn } = m;
    m.totalMain    = 0;
    db.members[idx] = m;
    writeDB(db);
    appendLog(kode, m.nama, "REWARD_GRATIS", `Reward ke-${totalGratis}`);
  ...
  db.members[idx] = m;
  writeDB(db);
  appendLog(kode, m.nama, expired ? "SCAN_RESET" : "SCAN", `Kunjungan ke-${m.totalMain}`);
